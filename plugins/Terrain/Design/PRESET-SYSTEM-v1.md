...
- **On load, absent ≠ clear** for `cardStates` (merge-only), `convIRRaw*`, and every `isNotEmpty()`-guarded blob (mod matrix, envelopes, LFO shapes, curves, arp lanes, MIDI map, macro names, sample paths). Only `wtImportPcm` clears on absence. A `.terrain` loader clears those members FIRST or preset B inherits preset A's mod matrix, cards and IRs. And on save, every empty-guarded property must be REPLACED unconditionally or `<preset>` metadata echoes into host sessions.
