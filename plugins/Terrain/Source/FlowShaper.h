...
public:
    ...
    void process (float* L, float* R, int n, double hostPpq, double bpm, bool playing) noexcept
    {
        const ShaperState* S = state_.load (std::memory_order_acquire);
        if (S == nullptr || n <= 0) return;
        const double BP = bpm > 0.0 ? bpm : 120.0, pps = BP / 60.0 / sr_, fpb = sr_ / (BP / 60.0);
        Ring* ring = ring_.load (std::memory_order_acquire);
        const ShaperLane& VL = S->lanes[0]; const ShaperLane& TL = S->lanes[1]; const ShaperLane& FL = S->lanes[2]; const ShaperLane& PN = S->lanes[3];
        const ShaperLane& RP = S->lanes[4]; const ShaperLane& DR = S->lanes[5]; const ShaperLane& PH = S->lanes[6]; const ShaperLane& CR = S->lanes[7];
        ...
        const float res = 1.0f - 0.95f * FL.k[0], kres = 2.0f * res;
        ...
        for (int i = 0; i < n; ++i, beat += playing ? pps : 0.0)
        {
            ...
            if (TL.on && ring != nullptr)
            {
                ...
                if (std::fabs (dBeh) > 4.0)
                {
                    if (! stepping_)
                    {
                        ...
                        tsXfN_ = std::max (48, (int) (sr_ * 0.001 * (0.5 + TL.k[0] * 4.5))); tsXf_ = tsXfN_;
                        double p2 = p + 1.5 / kShaperT; p2 -= std::floor (p2);
                        const float s2 = readShape (TL, p2);
                ...
                else
                {
                    ...
                    { tsOld_ = tsPos_ + 1.0; tsXfN_ = std::max (48, (int) (sr_ * 0.001 * (0.5 + TL.k[0] * 4.5))); tsXf_ = tsXfN_; }
                }
                ...
                tsPos_ = pos;
                l = l + (tl - l) * TL.blend; r = r + (tr - r) * TL.blend;
            ...
            if (DR.on)
            {
                ...
                const float s = readShape (DR, p) * DR.depth; const float g = 1.0f + 18.0f * s;
                const float mk = 1.0f / std::pow (g, 0.6f * (0.3f + 0.7f * DR.k[1]));
                ...
                const float tone = DR.k[0]; const float tc = 1.0f - std::exp (-2.0f * 3.14159265f * (400.0f + 12000.0f * tone * tone) / (float) sr_);
                ...
                l = l + (tl2 - l) * DR.blend; r = r + (tr2 - r) * DR.blend;
            ...
            if (CR.on)
            {
                ...
                const float s = readShape (CR, p) * CR.depth;
                const float bits = 16.0f - (4.0f + 8.0f * CR.k[0]) * s; const float q = std::pow (2.0f, bits - 1.0f);
                const int hold = 1 + (int) (s * (2.0f + 30.0f * CR.k[1]));
                if (holdN_ <= 0) { holdL_ = std::round (l * q) / q; holdR_ = std::round (r * q) / q; holdN_ = hold; }
                --holdN_;
                l = l + (holdL_ - l) * CR.blend; r = r + (holdR_ - r) * CR.blend;
            ...
            if (FL.on)
            {
                ...
                for (int c = 0; c < 2; ++c)
                {
                    ...
                    if (FL.k[1] > 0.0f) x = std::tanh (x * (1.0f + 6.0f * FL.k[1])) / (1.0f + FL.k[1]);
                    ...
                    if (c) r = r + (y - r) * FL.blend; else l = l + (y - l) * FL.blend;
                }
                vizPh_[2] = (float) p; vizV_[2] = s;
            }
            // ── PHASER: six allpasses whose centre the shape moves, with feedback ──
            if (PH.on)
            {
                const double cyc = kShaperRateBeats[PH.rate & 7]; double p = beat / cyc; p -= std::floor (p);
                const float s = readShape (PH, p);
                const float fb = 0.2f + 0.72f * PH.k[0];
                const float fc = 200.0f * std::pow (40.0f, s * PH.depth);
                const float st = PH.k[1] * 0.35f;   // stereo: the right channel's centre sits a little higher
                for (int c = 0; c < 2; ++c)
                {
                    ...
                    ch_[c].phfb = y;
                    if (c) r = r + y * PH.blend; else l = l + y * PH.blend;
                }
            ...
            if (PN.on)
            {
                ...
                const float s = readShape (PN, p); const float pan = (s * 2.0f - 1.0f) * PN.depth;
                float gl, gr;
                ...
                l *= 1.0f + (gl - 1.0f) * PN.blend; r *= 1.0f + (gr - 1.0f) * PN.blend;
            ...
            if (VL.on)
            {
                ...
                const float s = readShape (VL, p);
                const float gRaw = 1.0f - VL.depth * (1.0f - s);
                const float ms = 0.2f + 40.0f * VL.smooth; const float a = std::exp (-1.0f / ((float) sr_ * ms * 0.001f));
                smooth_[0] = gRaw + (smooth_[0] - gRaw) * a; const float g = 1.0f + (smooth_[0] - 1.0f) * VL.blend;
                l *= g; r *= g;
