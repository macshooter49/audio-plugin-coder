// ════════════════════════════════════════════════════════════════════════════
//  ResynthEngine_cpubench.cpp — CPU cost benchmark for the RESYNTH oscillator.
//  Proves the CPU cliff Max hit: measures cost/partial-sample, then computes what
//  the shared partial budget (kResynthPartialBudget) actually costs in % of one core.
//
//    c++ -std=c++17 -O3 -ffast-math -ISource Source/ResynthEngine_cpubench.cpp -o /tmp/rscb && /tmp/rscb
// ════════════════════════════════════════════════════════════════════════════
...
{
    ResynthFrameStore s; s.frames.resize (8); s.naturalSec = 2.0f; s.valid = true; s.f0 = 110.f;
    for (auto& f : s.frames)
    {
        int c = 0;
        for (int n = 1; n <= H && c < resynth::kMaxPartials; ++n)
...
static double benchInstance (int nEng, int budget, double quality, float stretch,
                             int blk, int blocksToRun, ResynthFrameStore& st, double sr,
                             int* outLivePartials, int unisonN = 1)
{
    std::vector<ResynthEngine> engs ((size_t) nEng);
    int liveBudget = 0;
    ResynthParams p; p.quality = (float) quality; p.stretch = stretch; p.scan = 0.5f;
    for (int i = 0; i < nEng; ++i)
    {
        ...
        ResynthParams pi = p; pi.start = (float) i / (float) std::max (1, nEng);
...
int main()
{
    ...
    std::printf ("=== RESYNTH (Resynth) CPU BENCHMARK ===\n");
    ...
    const int BUD = 640;   // = kResynthPartialBudget (rs2)
    ...
    std::printf ("------------------------------------------------\n");
    auto benchParam = [&] (const char* name, std::function<void(ResynthParams&)> setup)
    {
        std::vector<ResynthEngine> engs (16);
        int liveBudget = 0;
        ResynthParams p; p.quality = 1.0f; p.scan = 0.5f; setup (p);
        for (int i = 0; i < 16; ++i)
        { auto& e = engs[(size_t) i]; e.prepare (sr); e.setFrameStore (&st);
          e.setPartialBudget (&liveBudget, 640); e.setUnisonScale (16); ResynthParams pi = p; pi.start = i / 16.f;
    ...
    benchParam ("neutral (baseline)", [] (ResynthParams&){});
    benchParam ("FORMANT=1.0 (O(n^2))", [] (ResynthParams& p){ p.formant = 1.0f; });
    benchParam ("TILT=0.9 (pow/partial)", [] (ResynthParams& p){ p.tilt = 0.9f; });
    benchParam ("SHAPE=1.0 saw",         [] (ResynthParams& p){ p.shape = 1.0f; p.shapeTarget = 2; });
    // fb605 — this bench had rotted shut: it still set ResynthParams::cut, which rs2-cut RETIRED
    // (the one LP-or-HP CUT knob became the back row's two coexisting cuts, `lo` = HP corner and
    // `hi` = LP corner). It has not compiled since; ported to `hi`, same low-pass being measured.
    benchParam ("HIGH=LP 0.3",           [] (ResynthParams& p){ p.hi = 0.3f; });
    benchParam ("SIEVE=0.5",             [] (ResynthParams& p){ p.sieve = 0.5f; });
    benchParam ("DRIVE=1.0 (postproc)",  [] (ResynthParams& p){ p.drive = 1.0f; });
    benchParam ("CRUSH=0.9 (postproc)",  [] (ResynthParams& p){ p.crush = 0.9f; });
    benchParam ("ALL engaged",           [] (ResynthParams& p){ p.formant=1.f; p.tilt=0.9f; p.shape=1.f; p.hi=0.4f; p.sieve=0.3f; p.drive=0.7f; p.crush=0.5f; });

    std::printf ("\nNote: %% is of ONE core, for the Resynth oscillator ALONE (no filters/FX/other engines).\n");
