// ══════════════════════════════════════════════════════════════════════════════════════════════
//  TerrainNoise_test.cpp — tp82. THE NULL FOR LIFTING THE NOISE GENERATOR OUT OF SynthVoice.
//
//    g++ -std=c++17 -O2 -Wall -ISource Source/TerrainNoise_test.cpp -o /tmp/tn && /tmp/tn
//
//  The Shaper needed a Noise lane, and unlike every other effect it borrows there was no standalone
//  engine to borrow: the generator lived inside SynthVoice's render. Moving it is surgery on the
//  instrument, so the claim that has to be PROVEN is "the synth's noise did not change" — not argued.
//
//  `FrozenNoise` below is the generator EXACTLY as it stood before the move, pasted here and never
//  to be edited again. tw::TerrainNoise is the one the instrument and the Shaper now share. This
//  runs both, in lockstep, across all thirteen colours, and demands they agree BIT FOR BIT — not
//  "close", not within an epsilon: the same float. Anything less would not be a null.
//
//  ⚠️ If a future change to TerrainNoise.h reds this file, that is the point. Either the change was
//  meant to alter the instrument's noise (then Max has to hear it first, and this frozen copy is what
//  the old sound was), or it was not (then the change is wrong).
// ══════════════════════════════════════════════════════════════════════════════════════════════
...
// ── the generator as it was, frozen. DO NOT EDIT. ──────────────────────────────────────────────
namespace frozen
{
struct FrozenNoise
{
    static inline float tnClampF (float lo, float hi, float v) noexcept { return v < lo ? lo : (v > hi ? hi : v); }
            ...
            switch (noiseType_)
            {
                ...
                case 10: case 11: case 12: {   // Space — Chamberlin SVF resonant band-pass (tonal wash, not flat noise)
                    ...
                    if (noiseType_ == 10) {          // Space Open — broad airy wash, slow drift
                        ...
                        amp = 2.6f * tnClampF (0.0f, 1.4f,
                                      0.45f + 0.55f*noiseSine (windPh_) + 2.5f*gustL_);
                    }
                    float f = tnClampF (0.0f, 0.9f, fc * w0);   // f = 2·sin(π·fc/fs) ≈ 2π·fc/fs
...
int main()
{
    std::printf ("TerrainNoise — the null for lifting the generator out of SynthVoice\n\n");
    static const char* NM[13] = { "White", "Pink", "Brown", "Geiger", "Tape Hiss", "Tape Hum", "Tape Air",
                                  "Tape Crackle", "Clean Vinyl", "Dirty Vinyl", "Space Open", "Space Helium", "Space Wind" };
    const int N = 48000 * 2;   // two seconds a colour: long enough for the slow LFOs (0.05 Hz breath) to travel
    int worstType = -1; long long worstAt = -1; double worstDiff = 0;
    bool allSame = true, allMoved = true;
    for (int t = 0; t < 13; ++t)
    {
        tw::TerrainNoise now; frozen::FrozenNoise was;
        now.prepare (48000.0); now.setType (t);
        was.noiseSR_ = 48000.0f; was.noiseType_ = t;
        double peak = 0; bool same = true;
        for (int i = 0; i < N; ++i)
        {
            float aL = 0, aR = 0, bL = 0, bR = 0;
            now.tick (aL, aR); was.noiseTick (bL, bR);
            if (std::memcmp (&aL, &bL, sizeof (float)) != 0 || std::memcmp (&aR, &bR, sizeof (float)) != 0)
            { if (same) { same = false; worstType = t; worstAt = i; worstDiff = std::fabs ((double) aL - (double) bL); } }
            if (std::fabs (aL) > peak) peak = std::fabs (aL);
        }
        if (! same) allSame = false;
        if (peak < 1.0e-4) allMoved = false;   // a colour that makes no sound would pass a null trivially
        char buf[160]; std::snprintf (buf, sizeof buf, "%-13s bit-identical over %d samples (peak %.4f)", NM[t], N, peak);
        check (same && peak > 1.0e-4, buf);
    }
    char sum[220];
    std::snprintf (sum, sizeof sum, "ALL THIRTEEN COLOURS: moved, not rewritten%s",
                   allSame ? " — every sample identical" : " — DIVERGED");
    check (allSame && allMoved, sum);
    if (! allSame) std::printf ("  first divergence: type %d at sample %lld, |d| = %.9f\n", worstType, worstAt, worstDiff);

    // and the Shaper's own instance starts from the same silence, so its noise is the instrument's noise
    {
        tw::TerrainNoise a, b; a.prepare (48000.0); b.prepare (48000.0); a.setType (9); b.setType (9);
        for (int i = 0; i < 4800; ++i) { float x = 0, y = 0; a.tick (x, y); }
        a.reset();
        bool same = true;
        for (int i = 0; i < 4800; ++i)
        { float x = 0, y = 0, u = 0, v = 0; a.tick (x, y); b.tick (u, v); if (x != u || y != v) { same = false; break; } }
        check (same, "reset() returns a used generator to a fresh one's exact state (a second owner sounds like the first)");
    }
    std::printf ("\n%d checks, %d failed\n", g_checks, g_fail);
