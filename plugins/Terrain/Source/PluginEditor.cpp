...
// fb605 — THE RENAME REACHES THE FOLDER NAME, BUT IT MOVES NOTHING. The product is called
// **Terrain** now and this folder is USER-VISIBLE (the preset browser's "Locate Folder" and the
// wavetable imports button both hand a path under it straight to Finder/Explorer), so a NEW user
// must only ever see "Terrain". The owner already has 29 card presets, 67 MB of Noise and 32 MB of
// Blends under the OLD name, so the rule is PREFER-NEW / FALL-BACK-TO-OLD / CREATE-NEW, with no
// migration step and nothing renamed on disk:
//     WavesCrate/Terrain            exists -> use it   (a new user, or anyone already moved)
//     WavesCrate/TerrainInstrument  exists -> use it   (the owner's content — never touched)
//     neither                              -> Terrain  (the callers createDirectory() it)
// "TerrainInstrument" below is therefore an ADDRESS, not a name — same law as the GEODE param ids:
// rename the class, the files and the UI, never the thing already written to disk. ~/Library/
// WavesCrate holds one folder per product (TerrainChop, TerrainGlitch, ...), so "Terrain" is free
// here; the Noizefield and Music roots below are NOT, and resolve the other way round — read them.
// ⚠️ KEEP THIS BODY SELF-CONTAINED. Tests/extract_helpers.py slices it verbatim into the header
// Tests/preset_path_cert.cpp compiles, with juce_core and nothing else in scope, so a call out to
// a shared helper would leave the cert compiling something that is not what ships. That is also
// why PluginProcessor.cpp::terrainDataDirP() repeats these six lines instead of including them —
// the two are one rule in two TUs and MUST move together.
static juce::File terrainDataDir()
{
    const auto root   = juce::File::getSpecialLocation (juce::File::userApplicationDataDirectory)
                          .getChildFile ("WavesCrate");
    const auto fresh  = root.getChildFile ("Terrain");             // fb605 — what a new user sees
    const auto legacy = root.getChildFile ("TerrainInstrument");   // fb605 — the owner's content
...
//   Wavetables/             : userAppData/Noizefield/<resolved below>
//
// fb605 — terrainSettingsFile IS NOT RENAMED AND NEITHER IS ITS FOLDER. "Waves Crate"/Terrain is
// already the short name, and Terrain FX writes PluginSettings.json into that SAME directory: the
// FILENAME is what separates the two products there, not the folder. Nothing to do here.
...
// fb605 — THE NOIZEFIELD ROOT RESOLVES LEGACY-FIRST, AND THE ASYMMETRY WITH terrainDataDir() IS
// THE POINT. Everywhere else the new name wins; here it must not, because `Noizefield/Terrain` IS
// NOT FREE — Terrain FX has shipped its UserPresets.xml into it for as long as it has existed
// (plugins/TerrainFX/Source/PluginProcessor.cpp, getUserPresetsFile) and that file is sitting in it
// on the owner's machine right now. Both products write a <TerrainUserPresets> root with the same
// field names, so preferring the new spelling here would make the instrument silently ADOPT the
// FX's preset store and clobber it on the next save — precisely the collision
// PluginProcessor.cpp::getUserPresetsFile has carried a warning about since it was written. So:
//     Noizefield/"Terrain Instrument"  exists -> use it  (the owner's 5 patches + his Wavetables)
//     otherwise                               -> Noizefield/Terrain   (all a new user ever sees)
// The one case this does NOT cover is a fresh machine that already ran a PRE-rename Terrain FX
...
static juce::File terrainWavetablesDir()
{
    ...
    const auto legacy = nf.getChildFile ("Terrain Instrument");    // fb605 — the owner's Wavetables
    if (legacy.exists()) return legacy.getChildFile ("Wavetables");
    return nf.getChildFile ("Terrain").getChildFile ("Wavetables"); // fb605 — what a new user sees
}

// fb605 — THE CAPTURE / STEMS FOLDER, legacy-first for the same reason and with sharper teeth:
// ~/Music/"Waves Crate"/Terrain is Terrain FX's capture folder (73 of the owner's FX captures are
// in it; TerrainFX/Source/PluginProcessor.cpp writes Terrain_Capture_*.wav there) and the exporter
// deleteFile()s its target before writing, so pointing the instrument at it is not merely untidy.
//     Music/"Waves Crate"/"Terrain Instrument"  exists -> use it   (the owner's 246 files + Stems)
//     otherwise                                        -> Music/"Waves Crate"/Terrain
// PluginProcessor.cpp::terrainCaptureDirP() is the identical rule for the WAV the processor writes;
// the two must move together. This one is not sliced by any cert, but it is kept in the same shape.
static juce::File terrainCaptureDir()
{
    ...
    : audioProcessor (p)
{
    ...
    webView = std::make_unique<TerrainWebView>(
        juce::WebBrowserComponent::Options()
            ...
            .withOptionsFrom(lfoGlobalRelay).withOptionsFrom(lfoGlobalSyncRelay).withOptionsFrom(lfoGlobalDivRelay).withOptionsFrom(lfoGlobalRateRelay).withOptionsFrom(lfo10RateRelay).withOptionsFrom(lfo10DepthRelay).withOptionsFrom(lfo10ShapeRelay).withOptionsFrom(lfo10SyncRelay).withOptionsFrom(lfo10DivRelay).withOptionsFrom(lfo10PhaseRelay)
            ...
            .withOptionsFrom(synOscAHarmTableRelay).withOptionsFrom(synOscBHarmTableRelay).withOptionsFrom(synOscCHarmTableRelay).withOptionsFrom(synOscDHarmTableRelay)   // fb601 — HARM's own table roster
                                                     ...
                                                     juce::WebBrowserComponent::NativeFunctionCompletion complete)
            {
                ...
                if (args.size() >= 4)
                {
                    ...
                    if ((int) args[3] == 0 && ! TerrainAudioProcessor::physicalLeftButtonDown())
                    { complete ({}); return; }
                                                      ...
                                                      juce::WebBrowserComponent::NativeFunctionCompletion complete)
            {
                ...
                // timestamped WAV in <Music>/Waves Crate/<Terrain|Terrain Instrument>/Stems/.
                ...
                const auto destFolder = terrainCaptureDir().getChildFile ("Stems");   // fb605 — new name, old name honoured
            ...
            .withNativeFunction("exportAllStems", [this](const juce::Array<juce::var>&,
                                                          juce::WebBrowserComponent::NativeFunctionCompletion complete)
            {
                // Writes all 4 layer stems to <Music>/Waves Crate/<Terrain|Terrain Instrument>/Stems/.
    ...
    // Mod redesign Stage 2 — LFOs 2..5 attachments
    {
        ...
        // fb601 — HARM's own table selection. Bound exactly like SYN_OSC_x_WT_PRESET so the page can
        // drive it with the same Juce.getSliderState(...).setNormalisedValue(idx / (N - 1)) it uses
        // for the wavetable header — 46 choices, so the divisor is 45.
        mkAtt(synOscAHarmTableAttachment, ParameterIDs::SYN_OSC_A_HARM_TABLE, synOscAHarmTableRelay);
        mkAtt(synOscBHarmTableAttachment, ParameterIDs::SYN_OSC_B_HARM_TABLE, synOscBHarmTableRelay);
        mkAtt(synOscCHarmTableAttachment, ParameterIDs::SYN_OSC_C_HARM_TABLE, synOscCHarmTableRelay);
        mkAtt(synOscDHarmTableAttachment, ParameterIDs::SYN_OSC_D_HARM_TABLE, synOscDHarmTableRelay);
        ...
        mkAtt(lfoGlobalAttachment, ParameterIDs::LFO_GLOBAL, lfoGlobalRelay); mkAtt(lfoGlobalSyncAttachment, ParameterIDs::LFO_GLOBAL_SYNC, lfoGlobalSyncRelay); mkAtt(lfoGlobalDivAttachment, ParameterIDs::LFO_GLOBAL_DIV, lfoGlobalDivRelay); mkAtt(lfoGlobalRateAttachment, ParameterIDs::LFO_GLOBAL_RATE, lfoGlobalRateRelay); mkAtt(lfo10RateAttachment, ParameterIDs::LFO10_RATE, lfo10RateRelay); mkAtt(lfo10DepthAttachment, ParameterIDs::LFO10_DEPTH, lfo10DepthRelay); mkAtt(lfo10ShapeAttachment, ParameterIDs::LFO10_SHAPE, lfo10ShapeRelay); mkAtt(lfo10SyncAttachment, ParameterIDs::LFO10_SYNC, lfo10SyncRelay); mkAtt(lfo10DivAttachment, ParameterIDs::LFO10_DIV, lfo10DivRelay); mkAtt(lfo10PhaseAttachment, ParameterIDs::LFO10_PHASE, lfo10PhaseRelay);
    }
...
// terrainDataDir() (fb605: ~/Library/WavesCrate/Terrain, or .../TerrainInstrument where that is
// the folder that exists) is the path the import registry already persists to from FL (fb60), so
// it's writable where temp/App Support are not.
...
public:
    ...
    TerrainCardWindow (TerrainAudioProcessor& proc, const juce::String& cardId,
...
void TerrainUiCore::timerCallback()
{
    ...
    struct UiClock
    {
        TerrainAudioProcessor& p; long long t0;
        UiClock (TerrainAudioProcessor& pp)
    ...
    // 4 x 1024 floats at 3 dp — comfortably under the proven ~80 KB EQ push below.
    {
        ...
        if (oscActive && pushScopeW)   // fb486 whale gate (the park-push below is NOT gated)
        {
            ...
            constexpr int N = TerrainAudioProcessor::OSC_SCOPE_SIZE;
            float win[4][N];
    ...
    //    contract. Zero audio-thread cost; only fires while ≥1 osc is on Engine::SPEC (Geode).
    {
        ...
        if (sr > 0.0 && webView != nullptr)
        {
            ...
            for (int o = 0; o < 4; ++o)
            {
                ...
                const tw::ResynthFrameStore* store = audioProcessor.geodeLiveStore (o);
                if (engineIdx != 3 || store == nullptr) { s += "{on:0}"; continue; }   // 3 = Engine::SPEC
                const char* const* g = GID[o];
                tw::ResynthParams gp;   // display path: same REMAP as the audio gather (POSITION→start, etc.)
                ...
                // major bytes (x*IH+y, bin 0 = low freq), base64, → window.__geodeImage.
                {
                    ...
                    auto contentChanged = [] (const tw::ResynthParams& a, const tw::ResynthParams& b) noexcept
                    {
...
<style>
  ...
  /* ─── XY pad UI hidden in Terrain (2026-05-26) ─────────────────
...
void TerrainAudioProcessorEditor::onPeerArrived()
{
   #if JUCE_WINDOWS
    if (core_ != nullptr)
        if (auto* peer = getPeer())
...
void TerrainAudioProcessorEditor::replaceDeadCore()
{
    if (core_ != nullptr)
    {
        core_->detach();
...
namespace
{
    struct TiParkEntry { TerrainAudioProcessor* proc; juce::uint64 gen; bool evicting; };
    ...
    void tiRegistryErase (TerrainAudioProcessor* proc)
    {
...
void TerrainAudioProcessor::destroyUiCoreOnly()
{
...
void TerrainAudioProcessor::rebuildUiCoreNow()
{
...
juce::Component* TerrainAudioProcessor::ensureUiCoreComponent()
{
...
void TerrainAudioProcessor::parkUiCore()
{
...
void TerrainAudioProcessor::destroyParkedUiIfGen (juce::uint64 gen)
{
...
void TerrainAudioProcessor::releaseUiCoreForShutdown()
{
    tiRegistryErase (this);
    parkedGen_ = 0;
    uiCore_.reset();
