// ════════════════════════════════════════════════════════════════════════════
//  ResynthEngine_test.cpp — offline DSP proof for the RESYNTH oscillator.
...
static ResynthFrameStore makeStore (int H)
{
    ResynthFrameStore s; s.frames.resize (4); s.naturalSec = 2.0f; s.valid = true; s.f0 = 220.f;
    for (auto& f : s.frames)
    {
        int c = 0;
...
static double renderFund (ResynthParams p, ResynthFrameStore& st, double playHz, double sr)
{
    ResynthEngine e; e.prepare (sr); e.setFrameStore (&st); e.setParams (p); e.noteOn (playHz, 999);
...
int main()
{
    ...
    ResynthParams base; base.scan = 0.f;             // HOLD read-head → measure a stable frame
    ...
    { ResynthParams p = base; p.formant = 0.0f; check ("formant=0.0", renderFund (p, st, playHz, sr)); }
    { ResynthParams p = base; p.formant = 1.0f; check ("formant=1.0", renderFund (p, st, playHz, sr)); }
    { ResynthParams p = base; p.shape = 1.0f; p.shapeTarget = 2; check ("shape=saw", renderFund (p, st, playHz, sr)); }
    { ResynthParams p = base; p.shape = 1.0f; p.shapeTarget = 1; check ("shape=square", renderFund (p, st, playHz, sr)); }
    { ResynthParams p = base; p.shape = 1.0f; p.shapeTarget = 0; check ("shape=sine", renderFund (p, st, playHz, sr)); }
    ...
    { const char* shN[11] = { "sine","square","saw","triangle","pulse","hollow","organ","half","vowel","bright","metal" };
      for (int t = 3; t <= 10; ++t) { ResynthParams p = base; p.shape = 1.0f; p.shapeTarget = t;
        char nm[28]; std::snprintf (nm, sizeof nm, "shape=%s", shN[t]); check (nm, renderFund (p, st, playHz, sr)); } }
    { ResynthParams p = base; p.drive = 1.0f; check ("drive=1.0", renderFund (p, st, playHz, sr)); }
    ...
    { const char* dm[6] = { "saturate","bloom","glint","moire","foldback","ember" };
      for (int m = 1; m <= 5; ++m) { ResynthParams p = base; p.drive = 0.9f; p.driveMode = m;
    ...
    { const char* sm[6] = { "floor","sparse","cloak","flicker","rake","parity" };
      for (int m = 1; m <= 5; ++m) { ResynthParams p = base; p.sieve = 0.6f; p.sieveMode = m;
    ...
    { ResynthParams p = base; p.smear = 0.9f; check ("melt=0.9", renderFund (p, st, playHz, sr)); }
    { ResynthParams p = base; p.quality = 0.05f; check ("quality=trash", renderFund (p, st, playHz, sr)); }

    // ── rs7 — SAMPLER-PARITY region / loop / reverse / fades + constant-cost unison adopt ──────
    {
        auto mkEng = [&] (ResynthParams p) { ResynthEngine e; e.prepare (sr); e.setFrameStore (&st); e.setParams (p); e.noteOn (playHz, 42); return e; };
        std::vector<float> L (512, 0.f), R (512, 0.f);
        // region confinement: head stays inside [0.2,0.8]; forward loop settles into [0.3,0.6]
        { ResynthParams p; p.scan = 1.0f; p.regionStart = 0.2f; p.regionEnd = 0.8f;
          p.loopStart = 0.3f; p.loopEnd = 0.6f; p.loopMode = 1;
          ResynthEngine e = mkEng (p);
        ...
        { ResynthParams p; p.scan = 1.0f; p.loopMode = 2;
          ResynthEngine e = mkEng (p);
        ...
        { ResynthParams p; p.scan = 1.0f; p.fadeIn = 0.4f; p.loopMode = 0;
          ResynthEngine e = mkEng (p);
        ...
        { ResynthParams p; p.scan = 0.f;
          ResynthEngine anchor = mkEng (p), sib = mkEng (p);
    ...
    { ResynthParams p = base; p.crush = 0.8f; check ("crush=0.8", renderFund (p, st, playHz, sr)); }
    { ResynthParams p = base; p.hi = 0.4f; check ("high=LP.4", renderFund (p, st, playHz, sr)); }
    { ResynthParams p = base; p.lo = 0.6f; check ("low=HP.6", renderFund (p, st, playHz, sr)); }
    { ResynthParams p = base; p.sieve = 0.5f; check ("sieve=0.5", renderFund (p, st, playHz, sr)); }
    { ResynthParams p = base; p.tilt = 0.9f; check ("tilt=0.9", renderFund (p, st, playHz, sr)); }
    ...
    { ResynthEngine e; e.prepare (sr); e.setFrameStore (&st); e.setParams (base); e.noteOn (playHz, 7); e.renderBlockAdd (L0.data(), R0.data(), N); }
    { ResynthParams pc = base; pc.crush = 0.9f; ResynthEngine e; e.prepare (sr); e.setFrameStore (&st); e.setParams (pc); e.noteOn (playHz, 7); e.renderBlockAdd (L1.data(), R1.data(), N); e.postProcess (L1.data(), R1.data(), N); }
    ...
    {
        ResynthFrameStore ms; ms.frames.resize (4); ms.naturalSec = 2.0f; ms.valid = true; ms.f0 = (float) playHz;
        ...
        ResynthParams sp; sp.scan = 0.f; sp.shape = 1.0f; sp.shapeTarget = 2; sp.quality = 1.0f;   // full saw
        const int NN = 16384; std::vector<float> xs ((size_t) NN, 0.f), rr2 ((size_t) NN, 0.f);
        ResynthEngine e; e.prepare (sr); e.setFrameStore (&ms); e.setParams (sp); e.noteOn (playHz, 5);
        ...
        resynthdsp::fft (bf.data(), NN, false);
    ...
    {
        ResynthFrameStore sp; sp.frames.resize (2); sp.naturalSec = 1.0f; sp.valid = true; sp.f0 = (float) playHz;
        for (auto& f : sp.frames)
        { f.ratio[50] = 1.f; f.amp[50] = 0.30f; f.ratio[55] = 2.f; f.amp[55] = 0.15f; f.nPartials = 56; }
        ResynthParams p; p.scan = 0.f; p.quality = 0.5f;    // active ≈ 34 < 56 → keepLoudest engages
        ResynthEngine e; e.prepare (sr); e.setFrameStore (&sp); e.setParams (p); e.noteOn (playHz, 3);
    ...
    // harness: ResynthEngine_cpubench.cpp.) Threshold is generous so it won't false-fail on a slow
    // box but WILL catch the 40-60% cliff that shipped in rs1.
    {
        const int   blk = 128, banks = 256, BUD = 640;   // BUD mirrors kResynthPartialBudget
        const double blockNs = (double) blk / sr * 1e9;  // realtime ns/block (one core)
        std::vector<ResynthEngine> engs ((size_t) banks);
        int live = 0;
        ResynthParams wp; wp.quality = 1.0f; wp.stretch = 0.98f; wp.scan = 0.5f;      // STRETCH pad
        ...
        { auto& e = engs[(size_t) i]; e.prepare (sr); e.setFrameStore (&st);
          ...
          ResynthParams pi = wp; pi.start = (float) i / banks; e.setParams (pi);
