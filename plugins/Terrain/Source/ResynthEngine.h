#pragma once
// ════════════════════════════════════════════════════════════════════════════
//  tw::ResynthEngine — Terrain RESYNTH oscillator (Engine::SPEC)
//
//  fb605 — THIS ENGINE WAS CALLED "GEODE". It is now RESYNTH, everywhere, in code and
//  in the UI: the class, the namespace (resynth::), this file, the cert seams. ONE
//  EXCEPTION, AND IT IS NOT NEGOTIABLE — see the ParameterIDs banner and the
//  "(APVTS: GEODE_*)" notes on ResynthParams below:
//
//    ⛔ THE APVTS PARAMETER-ID STRINGS STAY LITERALLY "SYN_OSC_x_GEODE_*". ⛔
//
//  Those strings are written into every saved patch and every automation lane the owner
//  has. Renaming them does not "finish the job" — it silently wipes every Resynth setting
//  from every project that already exists. They are an on-disk format, not a name.
//
//  A SAMPLE resynthesizer: OFFLINE-analyze a dropped sound into per-frame sinusoidal
//  PARTIALS; per-voice RESYNTHESIZE it as an oscillator bank, played back polyphonically
//  with a START / STRETCH / SCAN read-head and a spectral-sculpt + degrade chain —
//  pitched to the key, NEVER detuned.
//
...
//  into a shared read-only ResynthFrameStore (processor double-buffers + atomic-
...
//    c++ -std=c++17 -O2 -Wall -Wextra -ISource Source/ResynthEngine_test.cpp -o /tmp/rsyn && /tmp/rsyn
// ════════════════════════════════════════════════════════════════════════════

// fb605 — CERT-SEAM RENAME TRIPWIRE. The compile-time mutation seams this header reads were
// GEODE_* and are now RESYNTH_* (same names, same meaning, GEODE→RESYNTH). A cert still passing
// -DGEODE_… would compile CLEAN with its mutation silently absent — i.e. it would report a PASS
// it never earned. Make that a hard error instead of a lie.
#if defined(GEODE_NO_DECLICK)        || defined(GEODE_FORMANT_PRETHIN) || defined(GEODE_EMBER_WARMTH)     \
 || defined(GEODE_DRIVE_MUT_APPEND)  || defined(GEODE_DRIVE_MUT_NOKEY) || defined(GEODE_MUT_SHAPE0)       \
 || defined(GEODE_NO_SPAWN)          || defined(GEODE_SPAWN_APPEND_ONLY)|| defined(GEODE_NO_LEVELMATCH)   \
 || defined(GEODE_NO_POSTTHIN)       || defined(GEODE_FORMANT_OLD_ENV) || defined(GEODE_FORMANT_CLAMP12)  \
 || defined(GEODE_MUT_FORMANT_PITCH) || defined(GEODE_NO_FORMANT_LEVEL)|| defined(GEODE_MUT_FORMANT0)     \
 || defined(GEODE_MUT_LO_CURVE)      || defined(GEODE_MUT_EXCLUSIVE)
 #error "fb605: this cert seam was renamed GEODE_* -> RESYNTH_* (e.g. -DGEODE_NO_DECLICK is now -DRESYNTH_NO_DECLICK). Update the -D; do NOT touch the SYN_OSC_x_GEODE_* PARAM IDs, which deliberately keep the word GEODE."
#endif
...
struct ResynthFrame
{
    std::array<float, resynth::kMaxPartials> ratio {}; // partial freq / fundamental
    std::array<float, resynth::kMaxPartials> amp   {}; // linear amplitude
    std::array<float, resynth::kNoiseBands>  noise {}; // residual band energies (linear)
    int nPartials = 0;
};

// Shared read-only analyzed store (processor-owned, per osc, double-buffered).
struct ResynthFrameStore
{
    std::vector<ResynthFrame> frames;
...
//
// fb605 — READ THIS BEFORE "FINISHING" THE GEODE→RESYNTH RENAME.
// The FIELD names below are the Resynth vocabulary. The "(APVTS: GEODE_*)" note on each one is
// the APVTS parameter ID it is gathered from (see the GEODE_IDS table in PluginProcessor), and
// EVERY ONE OF THOSE IDS STILL CONTAINS THE LITERAL WORD "GEODE" ON PURPOSE.
// A parameter ID is not a name — it is the on-disk key. It is written verbatim into every saved
// patch and into every automation lane in every DAW project. Change "SYN_OSC_A_GEODE_HAZE" to
// "…_RESYNTH_HAZE" and the host looks for a parameter that no longer exists: the setting is
// dropped on load and the automation lane goes dead. Silently. In already-shipped work.
// So: rename the class, the namespace, the file, the seams, the menus, the labels — NOT these.
// They stay GEODE forever, or until there is a written state-migration path that reads both.
struct ResynthParams
{
    ...
    bool operator== (const ResynthParams& o) const noexcept
    {
    ...
    bool operator!= (const ResynthParams& o) const noexcept { return ! (*this == o); }
};

// ═══ small self-contained spectral helpers (adapted from BlendEngine, offline) ═══
namespace resynthdsp {
...
inline void fft (std::complex<float>* x, int n, bool inverse) noexcept
{
    ...
    {
        const double ang = (inverse ? 2.0 : -2.0) * resynth::kPi / len;
...
inline const std::vector<float>& hann() noexcept
{
    static const std::vector<float> w = [] {
        std::vector<float> v ((size_t) resynth::kWin);
        for (int i = 0; i < resynth::kWin; ++i)
            v[(size_t) i] = 0.5f - 0.5f * std::cos (2.f * resynth::kPi * (float) i / (float) (resynth::kWin - 1));
        return v;
    }();
    return w;
}
...
// ═══ ResynthAnalyzer — OFFLINE, message-thread only (allocates freely) ═══════════
class ResynthAnalyzer
{
public:
    // Analyze a mono view of a sample into partials per frame (SAMPLE-ONLY resynth).
    static void analyzeSample (const float* mono, int n, double rate, ResynthFrameStore& out)
    {
        out = ResynthFrameStore {};
        if (mono == nullptr || n < resynth::kWin / 2 || rate <= 0.0) return;

        const float f0   = resynthdsp::detectF0 (mono, n, rate);
        const float fund = (f0 > 0.f) ? f0 : resynth::kRefHz;
        out.f0 = f0; out.fromWave = false;
        out.naturalSec = (float) ((double) n / rate);   // play-through reference: the sample's real length

        const int totalFrames = std::max (1, (n + resynth::kHop - 1) / resynth::kHop);
        const int stride      = std::max (1, (totalFrames + resynth::kMaxFrames - 1) / resynth::kMaxFrames);
        const auto& w         = resynthdsp::hann();
        const double binHz    = rate / (double) resynth::kWin;

        std::vector<std::complex<float>> buf ((size_t) resynth::kWin);
        std::array<float, resynth::kBins> mag {};
        Tracker T; T.reset();                 // persists across frames → stable partial slots
        std::vector<Peak> peaks; peaks.reserve ((size_t) resynth::kMaxPartials);

        for (int fi = 0; fi < totalFrames; fi += stride)
        {
            const int start = fi * resynth::kHop;
            for (int i = 0; i < resynth::kWin; ++i)
            {
                const int idx = start + i - resynth::kWin / 2;
                const float sv = (idx >= 0 && idx < n) ? mono[idx] : 0.f;
                buf[(size_t) i] = { sv * w[(size_t) i], 0.f };
            }
            resynthdsp::fft (buf.data(), resynth::kWin, false);
            for (int b = 0; b < resynth::kBins; ++b) mag[(size_t) b] = std::abs (buf[(size_t) b]);
    ...
    static void buildFromWave (const float* ratioFlat, const float* ampFlat,
                               int nFrames, int maxP, ResynthFrameStore& out)
    {
        out = ResynthFrameStore {};
        if (ratioFlat == nullptr || ampFlat == nullptr || nFrames < 1 || maxP < 1) return;
        out.fromWave = true; out.f0 = 0.f;
        const int fN = std::min (nFrames, resynth::kMaxFrames);
        for (int f = 0; f < fN; ++f)
        {
            ResynthFrame fr;
            int c = 0;
            for (int p = 0; p < maxP && c < resynth::kMaxPartials; ++p)
            {
...
private:
    ...
    {
        out.clear();
        const float med = frameMedian (mag, resynth::kBins);
        const float scale = 2.f / (float) resynth::kWin;
        for (int b = 2; b < resynth::kBins - 2; ++b)
        {
            const float v = mag[b];
            if (! (v > 3.f * med + 1e-12f && v >= mag[b - 1] && v >= mag[b + 1]
                   && v > mag[b - 2] && v > mag[b + 2])) continue;
            const int lo = std::max (0, b - 3), hi = std::min (resynth::kBins - 1, b + 3);
            double m = 0.0, c = 0.0;
        ...
        if ((int) out.size() > resynth::kMaxPartials)
        {
            std::nth_element (out.begin(), out.begin() + resynth::kMaxPartials, out.end(),
                              [] (const Peak& a, const Peak& b) { return a.amp > b.amp; });
            out.resize (resynth::kMaxPartials);
        }
    }
    ...
    struct Tracker
    {
        std::array<float, resynth::kMaxPartials> ratio {};
        std::array<float, resynth::kMaxPartials> amp   {};
        std::array<int,   resynth::kMaxPartials> miss  {};
        void reset() noexcept { ratio.fill (0.f); amp.fill (0.f); miss.fill (0); }
    };

    static void trackFrame (Tracker& T, std::vector<Peak>& peaks, ResynthFrame& fr) noexcept
    {
        ...
        int nP = 0; for (int s = 0; s < resynth::kMaxPartials; ++s) if (fr.amp[(size_t) s] > 1e-6f) nP = s + 1;
        fr.nPartials = nP;
    }

    static void extractNoise (const float* mag, double rate, ResynthFrame& fr)
    {
        const float med = frameMedian (mag, resynth::kBins);
        const double binHz = rate / (double) resynth::kWin;
        const double loF = 20.0, hiF = rate * 0.5;
        const double logLo = std::log (loF), logHi = std::log (std::max (loF + 1.0, hiF));
        for (int b = 1; b < resynth::kBins; ++b)
        {
            const bool strong = mag[b] > 4.f * med;
            if (strong) continue;
            const double hz = b * binHz;
            if (hz < loF) continue;
            int band = (int) ((std::log (hz) - logLo) / (logHi - logLo) * resynth::kNoiseBands);
            band = std::min (resynth::kNoiseBands - 1, std::max (0, band));
            fr.noise[(size_t) band] += mag[b] * mag[b];
        }
        for (int k = 0; k < resynth::kNoiseBands; ++k) fr.noise[(size_t) k] = std::sqrt (fr.noise[(size_t) k]);
    }

    static void normalize (ResynthFrameStore& s)
    {
        float pMax = 1e-9f, nMax = 1e-9f;
        for (auto& f : s.frames)
        {
            for (int i = 0; i < f.nPartials; ++i) pMax = std::max (pMax, f.amp[(size_t) i]);
            for (int k = 0; k < resynth::kNoiseBands; ++k) nMax = std::max (nMax, f.noise[(size_t) k]);
        }
        const float pG = 0.35f / pMax, nG = 0.5f / nMax;
        for (auto& f : s.frames)
        {
            for (int i = 0; i < f.nPartials; ++i) f.amp[(size_t) i] *= pG;
            for (int k = 0; k < resynth::kNoiseBands; ++k) f.noise[(size_t) k] *= nG;
        }
    }
};

// ═══ ResynthEngine — per-voice resynthesizer (real-time; allocates only in prepare) ═══
class ResynthEngine
{
public:
    ResynthEngine() = default;

    void prepare (double sampleRate) noexcept
    {
        rate_ = sampleRate > 0.0 ? sampleRate : 48000.0;
        (void) sineLUT();
        phase_.assign (resynth::kMaxPartials, 0.f);
        ampZ_.assign  (resynth::kMaxPartials, 0.f);   // declick: per-partial previous-block gain
        scatMul_.fill (1.f);
    ...
    void setFrameStore (const ResynthFrameStore* s) noexcept { store_ = s; }
    ...
    void setParams (const ResynthParams& p) noexcept
    {
    ...
    void noteOn (double playedHz, std::uint32_t seed) noexcept
    {
        ...
        for (int j = 0; j < resynth::kMaxPartials; ++j)
    ...
    bool prepareBank (int n) noexcept
    {
        ...
        int quota = resynth::kMinActive + (int) (clamp01 (p_.quality)
                        * (float) (resynth::kMaxActive - resynth::kMinActive));
        if (unisonDiv_ > 1) quota = std::max (std::min (quota, resynth::kUnisonFloor), quota / unisonDiv_);
    ...
    void adoptBank (const ResynthEngine& src) noexcept
    {
        wr_ = src.wr_;
            ...
            switch (p_.driveMode)
            {
                ...
                {                         // reorganizing as you push (metallic / West-Coast shimmer)
                    for (int i = 0; i < n; ++i)
                    {
                        ...
                        const float ang  = 0.5f * resynth::kPi * (1.f + driveSm_ * 6.f);
                ...
                case 5:                   // EMBER — asymmetric tube/tape bias: even harmonics, warm
                {
                    for (int i = 0; i < n; ++i)
                    {
                        ...
                        const float k = (12000.f - driveSm_ * 9500.f) * (float) (2.0 * resynth::kPi / rate_);
    ...
    bool computeFrameEnvelope (float pos01, float* outBins, int nBins, bool neutral) noexcept
    {
        ...
        if (! neutral)
        {
            ...
            const int quota = resynth::kMinActive + (int) (clamp01 (p_.quality)
                            * (float) (resynth::kMaxActive - resynth::kMinActive));
            const int active = std::min (quota, nP);
...
private:
    ...
    static const std::vector<float>& sineLUT() noexcept
    {
        static const std::vector<float> t = [] {
            ...
            for (int i = 0; i <= kLUT; ++i) v[(size_t) i] = std::sin (2.f * resynth::kPi * (float) i / (float) kLUT);
            return v;
        }();
        return t;
    }
    ...
    void interpFrame (float pos01, int nf) noexcept
    {
        ...
        const ResynthFrame& A = store_->frames[(size_t) fa];
        const ResynthFrame& B = store_->frames[(size_t) fb];
    ...
    void applySmear (float pos01, int nf) noexcept
    {
        ...
        const int nP = wr_.nPartials;
        float acc[resynth::kMaxPartials];                       // stack — never allocates on the audio thread
    ...
    int applyDriveChildren (int nP, int room) noexcept
    {
        ...
        {
            for (int j = 0; j < resynth::kMaxPartials; ++j) childKey_[(size_t) j] = -1;   // no children this block → nothing is owned
            return nP;
        }
        float ref = 1e-9f;
        ...
        const int keyBase = mode * resynth::kMaxPartials * 4;                         // a mode switch never inherits the other mode's slot
        bool owned[resynth::kMaxPartials];
        for (int j = 0; j < resynth::kMaxPartials; ++j) owned[j] = false;
        for (int j = nP; j < resynth::kMaxPartials; ++j) wr_.amp[(size_t) j] = 0.f;   // last block's children above the source
        ...
        auto add = [&] (int key, float r, float a) noexcept
        {
            ...
            int s = -1;
#ifdef RESYNTH_DRIVE_MUT_APPEND
            if (total < resynth::kMaxPartials) s = total; else return;                // cert seam: the shipped "slot after nP" placement → starved on a full bank
            wr_.ratio[(size_t) s] = r; wr_.amp[(size_t) s] = a; owned[s] = true; childKey_[(size_t) s] = key; ++total; --room; if (s + 1 > hi) hi = s + 1; return;
#endif
            for (int j = 0; j < resynth::kMaxPartials; ++j)                          // (1) my own slot from last block
#ifndef RESYNTH_DRIVE_MUT_NOKEY
...
#endif
            if (s < 0)
                for (int j = resynth::kMaxPartials - 1; j >= 0; --j)                 // (2) a SILENT free slot, top down
                    if (isFree (j) && ampZ_[(size_t) j] <= 1e-7f && childKey_[(size_t) j] < 0) { s = j; break; }
            if (s < 0)
                for (int j = resynth::kMaxPartials - 1; j >= 0; --j)                 // (3) any free slot (a dying voice — rare)
        ...
        {
            float tmp[resynth::kMaxPartials]; int n = 0;
            for (int j = 0; j < nP; ++j)
    ...
    {
        const float lnLo = std::log (resynth::kDispRMin), lnHi = std::log (resynth::kDispRMax);
        for (int j = 0; j < nP; ++j)
        {
            const float a = wr_.amp[(size_t) j];
            const float r = wr_.ratio[(size_t) j];
            if (a <= 1e-6f || r <= 0.f) continue;
            const float rr = r < resynth::kDispRMin ? resynth::kDispRMin : (r > resynth::kDispRMax ? resynth::kDispRMax : r);
    ...
    static inline float shapeWeight (int target, int n) noexcept
    {
        ...
        switch (target)
        {
            ...
            case 4:  return std::fabs (std::sin (fn * resynth::kPi * 0.25f)) / fn;    // Pulse  — |sin(nπd)|/n, d=0.25 (rs-shapefix: was 0.28 = 3.7 dB from Saw; 25 % pulse = 11.9 dB from Saw, every 4th harmonic notched)
            ...
            case 7:  return (n == 1) ? 0.5f : ((n & 1) ? 0.f : (2.f / resynth::kPi) / (fn * fn - 1.f)); // Half-wave
    ...
    int applySculpt (int nP, int quota) noexcept
    {
        ...
        if (shape > 1e-3f && nP > 0)
        {
            ...
            for (int j = nP; j < resynth::kMaxPartials; ++j) wr_.amp[(size_t) j] = 0.f;   // stale children/spawns of earlier blocks
            int hi = nP;
 ...
 #ifdef RESYNTH_SPAWN_APPEND_ONLY
                const int s = -1;
 #else
                const int s = resynth::kMaxPartials - kMaxH - 1 + n;
 #endif
            ...
            {
                while (cur < resynth::kMaxPartials && wr_.amp[(size_t) cur] > 0.f) ++cur;
                if (cur >= resynth::kMaxPartials) break;
                const int n = pend[k];
        ...
        {
#ifdef RESYNTH_FORMANT_OLD_ENV
            float snapR[resynth::kMaxPartials], snapA[resynth::kMaxPartials];   // cert seam: the SHIPPED estimator
...
#else
            float ex[resynth::kMaxPartials], ey[resynth::kMaxPartials];   // (ln r, ln a) of the live bank, sorted by r — stack, no RT alloc
            int ne = 0;
            for (int j = 0; j < nP; ++j)
            {
                const float a = wr_.amp[(size_t) j], r = wr_.ratio[(size_t) j];
        ...
        if (sieve > 1e-3f && nP > 0)
        {
            ...
            switch (p_.sieveMode)
            {
                ...
                {       //          (the literal MP3-at-low-bitrate / lossy-codec sound)
                    float lr[resynth::kMaxPartials];        // stack (fully overwritten below) — no RT alloc
                    for (int j = 0; j < nP; ++j)
    ...
    void keepLoudest (int nP, int keep) noexcept
    {
        ...
        float tmp[resynth::kMaxPartials];                       // stack — never allocates on the audio thread
    ...
    const ResynthFrameStore* store_ = nullptr;
    ResynthParams p_;
    ResynthFrame  wr_;                          // working (sculpted) partial bank
    ...
    std::array<float, resynth::kMaxPartials> scatMul_ { };           // hm2 — per-sibling static freq scatter
    std::array<int,   resynth::kMaxPartials> childKey_ { };          // fb598 — DRIVE child (parent slot × 4 + kind) owning each slot, -1 = none
};
