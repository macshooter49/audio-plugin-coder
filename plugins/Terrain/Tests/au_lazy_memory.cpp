...
// tp64 — and what malloc says is LIVE: a release of many small blocks stays resident in malloc's pools (the
//  footprint does not move) but leaves size_in_use — so both are printed, and the bank B bar reads this one.
static double liveMB() { malloc_statistics_t st {}; malloc_zone_statistics (nullptr, &st); return (double) st.size_in_use / 1e6; }
struct Au
{
...
int main()
{
    ...
    const double held2s = last, held2sLive = liveMB();
    a.pump (6.0); a.render (10); a.pump (0.5); step ("... 8 s later (the idle timer's release)");
    printf ("\n  net: %+.0f MB still held over the baseline\n", last - base);
    ...
    // tp64 — and bank B (E–H, +298 on its first build above) goes with them: the same window now returns the
    //  MODAL/HARM arms AND the second synthesiser. MODAL alone measured -2,082 in tp63; the bar wants that plus
    //  most of the bank.
    const double liveNow = liveMB();
    const bool bankBBack = (held2sLive - liveNow) > 2300.0;
    printf ("  %s  ... and BANK B goes with them when no E-H oscillator is on (malloc in use %+.0f MB over the window; MODAL alone is ~-2082)\n", bankBBack ? "PASS " : "FAIL ", liveNow - held2sLive);
    ...
    // tp64 — bank B comes back too: OSC A off, OSC E on, and E alone must sound
    a.setRaw ("Synth OSC A Engine", 0); a.setRaw ("Osc A Enable", 0); a.setRaw ("Osc E Enable", 1); settle(); a.pump (3.0); a.render (10); step ("OSC A off, OSC E on again (bank B re-armed, +3 s)");
    double eB = 0; size_t nB = 0;
    {
        std::vector<float> L (BLK), R (BLK);
        a.note (60, 100); a.note (64, 100);
        for (int b = 0; b < 80; ++b)
        {
            ...
            AudioUnitRenderActionFlags fl = 0; AudioTimeStamp ts {}; ts.mSampleTime = a.stamp; ts.mFlags = kAudioTimeStampSampleTimeValid;
            AudioUnitRender (a.au, &fl, &ts, 0, BLK, abl); a.stamp += BLK;
            if (b >= 10) for (int i = 0; i < BLK; ++i) { eB += (double) L[i] * L[i]; ++nB; }
            CFRunLoopRunInMode (kCFRunLoopDefaultMode, 0.002, false);
        }
        a.note (60, 0); a.note (64, 0);
    }
    const double rmsB = 20.0 * std::log10 (std::sqrt (eB / (double) (nB ? nB : 1)) + 1e-12);
    const bool soundsB = rmsB > -60.0;
    printf ("  %s  bank B (OSC E alone) sounds again after its release and a re-arm (%.1f dBFS)\n", soundsB ? "PASS " : "FAIL ", rmsB);
    a.setRaw ("Osc A Enable", 1); a.setRaw ("Osc E Enable", 0);
    ...
    printf ("\n");
    a.close(); return (gaveBack && sounds && bankBBack && soundsB) ? 0 : 1;
}
