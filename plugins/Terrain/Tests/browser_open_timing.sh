#!/bin/bash
# fb639 — build + run browser_open_timing.cpp against the built SharedCode (LTO archive), inside a fake bundle whose
...
F=$B/CMakeFiles/Terrain.dir/flags.make; OUT=${TMPDIR:-/tmp}/fb639_timing/Fake.bundle
...
eval clang++ -std=gnu++20 -O2 -arch arm64 -fvisibility=hidden -fvisibility-inlines-hidden -w $DEF $INC -c "$HERE/browser_open_timing.cpp" -o "$OUT/../t.o"
eval clang++ -arch arm64 -flto "$OUT/../t.o" "$B/Terrain_artefacts/Release/libTerrain_SharedCode.a" "$B/libTerrain_WebUI.a" $FW -o "$OUT/Contents/MacOS/browser_open_timing"
"$OUT/Contents/MacOS/browser_open_timing" 2>&1 | grep -v '^Terrain:'
exit ${PIPESTATUS[0]}
