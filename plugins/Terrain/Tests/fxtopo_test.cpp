...
constexpr uint16_t A = 1 << 0, B = 1 << 1, C = 1 << 2, D = 1 << 3,
                  SUB = 1 << 4, NOISE = 1 << 5;
constexpr uint16_t ALL = tw::FxChainTopology::kAllSrc;
...
struct Want { uint16_t entry; tw::SlotMask feed; bool consumed; uint16_t eff; };   // fb420 — 128-bit, 78 slots
...
int main()
{
    ...
    { uint16_t m[] = { (uint16_t) (A | C), C }; t.build (m, 2);
      expect (t, 0, { (uint16_t) (A | C), 0,       true,  (uint16_t) (A | C) });
      expect (t, 1, { 0,                 FROM(0), false, (uint16_t) (A | C) }); }
    ...
    { uint16_t m[] = { (uint16_t) (A | B), B, A }; t.build (m, 3);
      expect (t, 0, { (uint16_t) (A | B), 0,       true,  (uint16_t) (A | B) });
      expect (t, 1, { 0,                 FROM(0), true,  (uint16_t) (A | B) });
      expect (t, 2, { 0,                 FROM(1), false, (uint16_t) (A | B) }); }
    ...
    { uint16_t m[] = { (uint16_t) (A | C), A }; t.build (m, 2);
      expect (t, 0, { (uint16_t) (A | C), 0, true, (uint16_t) (A | C) });
      uint16_t m2[] = { A, (uint16_t) (A | C) }; t.build (m2, 2);
      expect (t, 0, { A,  0,       true,  A });
      expect (t, 1, { C,  FROM(0), false, (uint16_t) (A | C) }); }

    // ── 8. one source routed to three devices is still summed only ONCE.
    head ("8. A routed to three devices taps the oscillator once");
    { uint16_t m[] = { A, A, A }; t.build (m, 3);
      expect (t, 0, { A, 0,       true,  A });
    ...
    { uint16_t m[] = { (uint16_t) (A | C), C }; t.build (m, 2);
      expect (t, 0, { (uint16_t) (A | C), 0,       true,  (uint16_t) (A | C) });
      check (t.eff[1] == (uint16_t) (A | C), "downstream C device receives the fused A+C"); }
    ...
    { uint16_t m[] = { A, C, A, C, A, C }; t.build (m, 6);
    ...
    // ── tp20. THE SECOND BANK. Oscillators E–H are source bits 6..9 of the SAME mask. A card on A
    //       and a card on E are two lines that never touch (the same law as 13); a card on A|E fuses
    //       them because the user lit both pills. Bank 0 reads bits 0..5 back, bank 1 bits 6..9.
    head ("tp20. E (bank 1, bit 6) is an ordinary source: A and E never collapse; A|E fuses");
    { constexpr uint16_t E = 1 << tw::FxChainTopology::kBank1Shift;
      uint16_t m[] = { A, E }; t.build (m, 2);
      expect (t, 0, { A, 0, false, A });
      expect (t, 1, { E, 0, false, E });
    ...
    { uint16_t m[] = { (uint16_t) (A | B) }; t.build (m, 1);
      const uint16_t inherited = t.eff[0];                 // what the UI would copy
      uint16_t m2[] = { (uint16_t) (A | B), inherited }; t.build (m2, 2);
      expect (t, 0, { (uint16_t) (A | B), 0,       true,  (uint16_t) (A | B) });
      expect (t, 1, { 0,                 FROM(0), false, (uint16_t) (A | B) });
    ...
    { uint16_t m[] = { (uint16_t) (SUB | NOISE), D, SUB }; t.build (m, 3);
      expect (t, 0, { (uint16_t) (SUB | NOISE), 0, true,  (uint16_t) (SUB | NOISE) });
      expect (t, 1, { D,                       0, false, D });
      expect (t, 2, { 0, FROM(0), false, (uint16_t) (SUB | NOISE) });
    ...
    head ("17. model invariants hold for all mask combinations");
    {
        ...
        const uint16_t alphabet[] = { 0, A, C, (uint16_t) (A | C), (uint16_t) (A | B | C), ALL };
        ...
        for (int k = 0; k < NA; ++k)
        {
            ...
            for (int c = 0; c < 3; ++c)
            {
                ...
                if ((uint16_t) (t.eff[c] & t.entry[c]) != t.entry[c]) ++effBad;
    ...
    {
        uint16_t masks[50]; for (int i = 0; i < 50; ++i) masks[i] = A;
    ...
    {
        constexpr int N = 78;
        uint16_t masks[N]; for (int i = 0; i < N; ++i) masks[i] = A;
