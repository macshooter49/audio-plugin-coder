#!/bin/bash
# tp107 — build + run Tests/osc_solo_mute_cert.cpp against the built SharedCode (an LTO archive), with the
# Organics library pointed at the frozen fixture (Tests/fixtures/organics). The expression_midi.sh recipe.
#   Tests/osc_solo_mute_cert.sh            (build Terrain_VST3 / Terrain_AU first). Exit 0 = PASS.
# Run it on 4075f80c to see the E–H solo bug: bank-local anySolo fails [1] on every engine.
set -e
HERE=$(cd "$(dirname "$0")" && pwd); ROOT=$(cd "$HERE/../../.." && pwd); B=${TERRAIN_BUILD:-$ROOT/build}/plugins/Terrain
F=$B/CMakeFiles/Terrain.dir/flags.make; OUT=${TMPDIR:-/tmp}/tp107_solomute/Fake.bundle
...
eval clang++ -std=gnu++20 -O2 -arch arm64 -fvisibility=hidden -fvisibility-inlines-hidden -w $DEF $INC -c "$HERE/osc_solo_mute_cert.cpp" -o "$OUT/../t.o"
eval clang++ -arch arm64 -flto "$OUT/../t.o" "$B/Terrain_artefacts/Release/libTerrain_SharedCode.a" "$B/libTerrain_WebUI.a" $FW -Wl,-U,__ZN2tw14organics_debug17lastRenderReadersEv -Wl,-U,__ZN2tw14organics_debug15lastLiveReadersEv -Wl,-U,__ZN2tw14organics_debug14lastNoteRegionEv -Wl,-U,__ZN2tw14organics_debug6stealsEv -o "$OUT/Contents/MacOS/osc_solo_mute_cert"   # the runtime hooks are weak (absent with the stub)
export TERRAIN_ORGANICS_DIR="${TERRAIN_ORGANICS_DIR:-$HERE/fixtures/organics}"
"$OUT/Contents/MacOS/osc_solo_mute_cert" "$@" 2>&1 | grep -v '^Terrain:'
exit ${PIPESTATUS[0]}
