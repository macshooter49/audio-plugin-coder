/* tp36e — WHICH KNOB MAKES A VOICE SIX TIMES DEARER? Crazy rolls, cap off, chord held; every setSynParam the roll writes is
...
(function(){
  var R = { phase: 'start', rolls: [], errs: [], t0: performance.now() }; window.__tp36e = R;
  ...
  setInterval(function(){ if (!chunks.length) return; document.title = 'tp36e:' + gen + ':' + ci + '/' + chunks.length + ':' + chunks[ci]; ci = (ci + 1) % chunks.length; }, 250);
  ...
  function series(n){ return function(){ window.__tpDiceCap(40); window.__tpDiceLevelSet('crazy'); var seq = Promise.resolve();
