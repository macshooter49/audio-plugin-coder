#!/bin/bash
# tp114 — build + run Tests/modal_samples_cert.cpp against the built SharedCode (an LTO archive), with the
# Organics library pointed at the frozen fixture (Tests/fixtures/organics). The expression_midi.sh recipe.
#   Tests/modal_samples_cert.sh            (build Terrain_VST3 / Terrain_AU first). Exit 0 = PASS.
set -e
HERE=$(cd "$(dirname "$0")" && pwd); ROOT=$(cd "$HERE/../../.." && pwd); B=${TERRAIN_BUILD:-$ROOT/build}/plugins/Terrain
F=$B/CMakeFiles/Terrain.dir/flags.make; OUT=${TMPDIR:-/tmp}/tp114_modalsamples/Fake.bundle
