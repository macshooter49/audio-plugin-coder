...
//     ~/Library/Caches/<component binary> in EVERY host — dladdr resolves to the COMPONENT
//     binary, not the host process. fb605: that binary is "Terrain" now (it was "Terrain
//     Instrument"). DERIVED, not persisted — nothing the owner saved lives there, so it moves
//     with the rename. TERRAIN_CACHE_DIR overrides it for a pre-fb605 build.
