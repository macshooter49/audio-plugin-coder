#!/bin/bash
# tp103 — build + run settings_perf_cert.cpp against the built Terrain SharedCode (an LTO bitcode archive).
...
OUT=${TMPDIR:-/tmp}/terrain_settings_perf
...
eval clang++ -std=gnu++20 -O2 -arch arm64 -fvisibility=hidden -fvisibility-inlines-hidden -w $DEF $INC -c "$HERE/settings_perf_cert.cpp" -o "$OUT.o"
