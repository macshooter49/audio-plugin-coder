#!/bin/bash
# fb636e — build + run capture_flow_render.cpp against the built Terrain SharedCode (an LTO bitcode archive).
# Build Terrain_VST3 / Terrain_AU first. Exit 0 = PASS. CAPFLOW_OFF=1 runs the FLOW-off reference.
set -e
HERE=$(cd "$(dirname "$0")" && pwd)
ROOT=$(cd "$HERE/../../.." && pwd)
B=$ROOT/build/plugins/Terrain
F=$B/CMakeFiles/Terrain.dir/flags.make
OUT=${TMPDIR:-/tmp}/terrain_capflow
...
eval clang++ -std=gnu++20 -O2 -arch arm64 -fvisibility=hidden -fvisibility-inlines-hidden -w $DEF $INC -c "$HERE/capture_flow_render.cpp" -o "$OUT.o"
