...
struct Beacon
{
    ...
    Beacon()
    {
        ...
        // fb605 — the component binary is "Terrain" now. This beacon only READS, so the old name
        // stays in the list: a pre-fb605 bundle still reports instead of silently reading nothing.
        paths.push_back ([caches stringByAppendingPathComponent: @"Terrain/terrain-cpu.txt"]);
...
int main (int argc, char** argv)
{
    ...
    @autoreleasepool
    {
        ...
        bc.clean();
        NSString* caches = NSSearchPathForDirectoriesInDomains (NSCachesDirectory, NSUserDomainMask, YES)[0];
        // fb605 — check BOTH cache dirs: a hook left in the pre-rename one is still a leak.
        NSString* hook    = [caches stringByAppendingPathComponent: @"Terrain/terrain-ui-exp.js"];
        NSString* hookOld = [caches stringByAppendingPathComponent: @"Terrain Instrument/terrain-ui-exp.js"];
        const bool hookLeft = [[NSFileManager defaultManager] fileExistsAtPath: hook]
                           || [[NSFileManager defaultManager] fileExistsAtPath: hookOld];
