...
//   2  WHILE NOTES SOUND: 0 painter frames (tp105 — events only), per family type (bowed strings, plucked, winds, brass, keys, a twin)
//      — and the picture actually reacted (keys lit, strings rang, pads closed, valves pressed, bars struck). tp105: NO
//      emission from any bell (Max: the sax "noise") — no .wave element may appear
//   3  AFTER THE LAST EVENT: the painter parks within 500 ms
//   4  ONE INSTRUMENT BUILD ≤ 3 ms (render + DOM insert, uncached), every family, warm — a twin (two drawings) ≤ 5 ms
// ══════════════════════════════════════════════════════════════════════════════════════════════
...
(async () => {
  ...
  const rest = await p.evaluate(async () => { const A = window.__orgAnim, f0 = A.frames; await new Promise(r => setTimeout(r, 2000));
    return { frames: A.frames - f0, raf: A.raf, tmr: A.tmr, ringing: A.ringing.size }; });
  ...
  const runs = [];
  for (const [id, sel] of [['vsco.violin.section', '.r-string.ring'], ['freepats.nylon', '.r-string.ring'], ['vsco.flute', '.r-pad.on'], ['vcsl.vibraphone', '.r-bar.on'], ['vsco.trumpet', '.r-valve.on'], ['salamander.grand', '.r-key.on']]) {
    const r = await p.evaluate(async (id, sel) => { const sleep = ms => new Promise(r => setTimeout(r, ms)); await window.__orgSetInstrument('a', id); await sleep(300);
      const A = window.__orgAnim, f0 = A.frames, t0 = performance.now(); let seen = 0, wave = 0;
      for (let k = 0; k < 30; k++) { const base = 55 + (k >> 2) * 2;   // a chord that moves every 4 frames (new notes on, old ones off)
        ...
        if (document.querySelector('#osc-a-device .org-viz ' + sel)) seen++; if (document.querySelector('#osc-a-device .org-viz .wave')) wave++;
        await sleep(1000 / 15); }
      ...
      return { id, fps: +fps.toFixed(1), frames, reacted: seen, wave, parkMs: Math.round(lastAt - tLast), armed: !!(A.raf || A.tmr) }; }, id, sel);
    runs.push(r);
  }
  ok(runs.every(r => r.frames === 0 && r.reacted > 0 && r.wave === 0),
     '[2] while notes sound: the picture reacts (strings light, pads close, valves press, bars strike, keys light) with NO per-frame painter (tp105: events only — the 30 fps string wobble cost +11 % of a core in the real WebView) and NOTHING leaves a bell',
     runs.map(r => `${r.id}: ${r.fps} fps (${r.frames}) reacted ${r.reacted}/30 waves ${r.wave}`).join(' · '));
  ...
  const worst = Object.entries(bench).sort((a, b) => b[1] - a[1]), twins = await p.evaluate(() => Object.keys(window.__orgArt.families).filter(f => window.__orgArt.families[f].twin));
  const over = worst.filter(([k, v]) => v > (twins.indexOf(k) >= 0 ? 5 : 3));
  ok(over.length === 0 && Object.keys(bench).length >= 24, `[4] one instrument build ≤ 3 ms, a twin ≤ 5 ms (median of 7, warm), all ${Object.keys(bench).length} families (worst ${worst[0][0]} ${worst[0][1]} ms)`,
