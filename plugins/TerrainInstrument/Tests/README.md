...
| `wt_stretch_cert.cpp` | **fb583 — STRETCH MOVES THE PARTIALS, AND WT POS CANNOT FAKE IT** (offline, the real factory bank). Max rejected fb582's Spread on first listen — *"whenever I move my wave table to the same position that I move my spread, it sounds the exact same"* — and that cert had passed, because it asked "did the sound change?", which ANY knob touching the table answers yes to. So the central bar here is **imitability**: for a stretched table, sweep WT Pos across all 101 positions, find the closest spectral match, and require it to still be FAR. Metric is amplitude-weighted magnitude-spectrum distance in dB (a HEARING metric, fb283 — sample-difference RMS is banned). 13 bars: the fundamental is pinned exactly at every stiffness; the ladder never crosses; neutral is BYTE-identical on all 46 tables; **not imitable by WT Pos** (33.5 dB at full, 1.01x the whole WT Pos axis); **not the Spectral dropdown's two stretch modes either** (16.9 dB from the closest of 80 settings); no dead zone (worst 5% step 5.96 dB); the top half earns its place; 130 of 138 bank cells move and the 8 that don't are provably single-partial or band-filling frames; **no frame collapse** (+3.6 dB extra WT Pos level unevenness — a FIXED stiffness read +71.1 dB here, which is the defect this design exists to avoid); no aliasing (-164 dBc above the mip ceiling); the transform is 0.19 ms against the 4.6 ms bake it rides. ⚠️ Proven able to fail: a no-op transform reds 5 bars including both imitability bars; an unpinned fundamental reds bar 0. |
| `wt_stretch_au.cpp` | **fb583 — THE INSTALLED PLUGIN REALLY APPLIES STRETCH** (hosted AU). The offline cert proves the transform; this proves the WIRING, which is the fb469 failure class ("a display feed is not a control signal" built clean and looked wired). 5 bars on the shipping AU: the knob is exposed under its new name; **at ONE voice it still works** (moves 0.335 against a 0.019 noise floor, 17.3x — fb582's fatal flaw was needing unison); it is timbre not level (3.25 dB); the pitch does not move (0 DFT bins); no clicks while sweeping (2.71x the held-still step). ⚠️ Proven able to fail: a build with `stretching = false` in rebuildMorphIfNeeded reds bars 1 and 2 (0.015 against a 0.029 floor). |
