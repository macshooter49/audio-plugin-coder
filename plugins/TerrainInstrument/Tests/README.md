...
| `wt_spread_cert.cpp` | **fb582 — SPREAD GIVES EVERY UNISON VOICE ITS OWN FRAME, AND IT IS AUDIBLE** (installed AU). Max: "I don't like blur, never did... spread needs to DO something." OSC A alone on a real table with unison DETUNE at 0, so the frame each voice reads is the only thing separating them; two renders compared as normalised magnitude spectra (256-bin DFT, unit energy) so the number is TIMBRE, not level. ⚠️ The synth is not repeatable note to note by design (per-voice drift and start phase): one note gives a 5 dB sample-wise difference at the SAME settings, and eight near-identical voices comb differently every note (spectral 0.39). So every bar averages six notes and is read against a measured noise floor. Bars: at one voice spread is inert (0.0004 against a 0.0009 floor); at eight voices it moves the spectrum 0.58 against a 0.19 floor; the level moves 0.36 dB while it does; the knob is progressive (0.34 < 0.47 < 0.58); no clicks while sweeping under a held note. Proven able to fail: a build where `spreadFrame` returns the centre frame reads 0.117 against a 0.292 floor and bars 2 and 4 go red. |
