...
| `fb563_menu.js` | fb563 — THE CONTROL MENU: every destination opens exactly one house menu; the Modulate picker (six families, multi-select); route rows (depth slider · curve · remove · bypass · scale by); registry rows; reset; copy/paste; the wire codec; MIDI Learn's row + natives. 4 mutations, `FB563_MUTATE=1..4`. | 43 |
| `mod_src_cert.cpp` | fb563 — the six new SOURCES reach the audio on the installed AU (macro · wheel · aftertouch · bend · random · alt), the door drops unknown codes, depth sign, the connection curve, bypass, scale by. Level A at ZERO so the source is the only way to a sound. | 13 |
