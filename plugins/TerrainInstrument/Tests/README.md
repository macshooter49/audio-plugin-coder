...
| `wt_feedback_cert.cpp` | **fb584 — ONE KNOB, BOTH FEEDBACKS, AND BOTH HALVES OF THE LIFEGUARD LAW** (offline, the real factory bank). The oscillator modulates its own read through the DX7 mean filter our FM operator already uses (`y = 0.5(x[n]+x[n-1])`), bending BOTH the phase it reads at and the frame it reads from. The two paths are tapered differently on purpose, and that taper IS the law: measured, they fail in opposite directions — FRAME feedback is clean and progressive but alone reaches only 0.45x the WT Pos axis, while PHASE feedback reaches 1.6x but *its character is its alias* (filtering it clean drops it from 26.5 dB to 12.5). So frame rides a LINEAR taper and owns the bottom; phase rides a FOURTH-POWER taper and owns the top. 5 bars: the bottom is clean on every table at every octave (worst -27.5 dB at 10-25%); the top is violent (+14.6 dB inharmonic — the partials have left the grid); not imitable by WT Pos at full (mean **1.07x** the table's own WT Pos span, worst 0.78x); no dead zone (quietest step 1.7 dB, 0 of 20 inert); level swing 3.78 dB. ⚠️ 4x oversampling was built, measured and REJECTED — it costs 4x the oscillator, drops the level 10 dB, and cuts distinctiveness from 1.38 to 0.73, because here the dirty version IS the product. ⚠️ Proven able to fail: zeroing both depths reds bars 2, 3 and 4. |
| `wt_feedback_au.cpp` | **fb584 — FEEDBACK ON THE INSTALLED PLUGIN** (hosted AU). The offline cert proves the shape; this proves the wiring, which is the fb469 failure class (built clean, looked wired). Inharmonic energy measured against a Blackman-Harris window (-92 dB sidelobes), harmonics taken as ±4 bins around each k·f0. 8 bars: exposed as 'Feedback' and 'Spread' is gone; **works at ONE voice** (11.3x the noise floor — the flaw that killed fb582's Spread); the bottom is clean (-82.6 dB at 10%, -82.5 at 25%); the top is violent (-18.4 dB, from -82.7 at rest); progressive with no quarter going backwards; **no clicks** (sweeping is 0.91x the step of holding at the destination); level holds within 1.2 dB. ⚠️ The click bar's reference is the sweep's DESTINATION, not its origin — comparing a sweep that ends in genuine chaos against a clean FB=0 tone measures the feature working and calls it a click (that mistake read 7.54x). ⚠️ Proven able to fail: a build with the feature disabled reds bars 1, 3 and 4 (bar 3 reads -80.0 dB instead of -18.4). |
