...
public:
    ...
    void prepare (double sampleRate, int /*maxBlock*/) noexcept
    {
        ...
        retK_  = 1.0f / (0.030f * fs_);                      // the retrig residual ramp, 30 ms LINEAR
    ...
    void reset() noexcept
    {
        ...
        for (int v = 0; v < kMaxVoices; ++v) { resW_[v] = 0.0f; resPw_[v] = 0.0f; }
        retR_ = 0.0f; resBs_ = resBc_ = resBw_ = resGs_ = resGw_ = 0.0f;
    ...
    void processStereo (float* L, float* R, int n) noexcept
    {
        ...
        while (i < n)
        {
            ...
            for (; i < n; ++i)
            {
                ...
                glideGeom();
                // the retrig residual ramp: LINEAR to exactly zero (a one-pole parks).
                if (retR_ > 0.0f) { retR_ -= retK_; if (retR_ < 0.0f) retR_ = 0.0f; }
...
private:
    ...
    static constexpr CharSpec CHAR[kNumTypes][kNumChars] =
    {
        ...
        { { 1.00f, 1.00f, 1.00f, 0.00f, 1.0f, 1.00f,  0.0f, 0.0f, 1.000f, 0 },                        // Lilt
          ...
          { 1.00f, 1.00f, 1.00f, 0.00f, 1.0f, 1.00f,  0.0f, 1.0f, 1.000f, kStaticWalk },              // Static Pair (x3 = the static offset as a FRACTION of amtC, fb425)
    ...
    // 🔴 fb425 — 24 AND 16 WERE THE CEILINGS THAT MADE `Voices` PLATEAU. The count law
    //    below gives every Character its own STARTING count and then adds a fixed step per
    //    click of the knob, so the knob always travels; that needs headroom above the
    //    deepest Character's start (`Deep Twelve` starts at 18 stages and must still climb
    //    six clicks) or the ceiling eats the top of the knob exactly as it did before.
    static constexpr int kMaxAP = 36, kMaxBands = 20;
    ...
    void recalc() noexcept
    {
        ...
        for (int v = 0; v < kMaxVoices; ++v)
        {
            ...
            // 🔴 fb425 — `Static Pair`'s offset was the CONSTANT C.x3 (8 cents), which made
            //    Twofold's own hero knob DEAD on that Character: `Sway` measured 1.69 ->
            //    1.69 cents across its entire travel, the detector's own floor at both ends.
            //    Static means the pair does not MOVE, never that it has no width — so the
            //    offset is the cents the walk would have reached at this Amount, held still.
            if ((C.flags & kStaticWalk) && v > 0) sc = amtC * C.x3 * ((v & 1) ? 1.0f : -1.0f);
        ...
        // 🔴 fb425 — THE COUNT LAW. `nAP = round(x1 * nV)` multiplies the Character's depth
        //    INTO the knob, so a deep Character hits the ceiling a quarter of the way up and
        //    the top three quarters of `Voices` are bit-for-bit nothing: `Deep Twelve` ran
        //    18/24/24/24/24 stages across the whole knob and `Plush`/`Opposed` 12/16/24/24/24.
        //    Measured on the output, three quarters of that knob moved NOTHING on three of
        //    eight Characters — and Character 0 (the only cell fb424 swept) was clean, which
        //    is precisely why nobody saw it. The Character now sets where the knob STARTS and
        //    the knob adds a fixed step per click, so it travels its whole length on every
        //    Character by construction.
        {   const int baseAP = clampi ((int) std::lround (CHAR[type_][char_].x1 * 3.0f), 3, kMaxAP - 15);
            nAP_ = clampi (baseAP + 3 * (nV_ - 3), 2, kMaxAP); }
        ...
        // 🔴 fb425 — THE SAME COUNT LAW, AND HERE THE EVEN-ING WAS THE SECOND HALF OF THE
        //    BUG. round(x1*1.6*(nV-1)) forced even gave 2/4/4/6/6 bands on the six x1 = 0.55
        //    Characters: the even-ing collapsed 3->4 and 5->6, so TWO of the four quarters
        //    changed nothing at all (measured 0 / 47.8 / 0 / 43.9 — a perfect comb of dead
        //    steps). The band count is now built EVEN from the start, one whole band-pair per
        //    click, off a per-Character base — so it is even by construction, distinct at
        //    every click by construction, and never clamped at either end.
        {   const int halfBase = clampi ((int) std::lround (CHAR[type_][char_].x1 * 2.0f), 1, kMaxBands / 2 - 5);
            nB_ = clampi (2 * (halfBase + (nV_ - 3)), 2, kMaxBands); }
    ...
    void procVoices (float limHi, float& wetL, float& wetR) noexcept
    {
        ...
        for (int v = 0; v < kMaxVoices; ++v)
        {
            ...
            else if (T.mod == 3)                               // ── Twofold: random WALK
            {
                ...
                tickWalk (wk_[v], bwHz);
                float s = wk_[v].st + bleedWalkRes (v);        // fb425 retrig residual
                // `Static Pair` forces the walk to zero, so there is no trajectory to
                // restart and no residual to owe: the retrigger is INERT on that cell by
                // construction, declared as such in §F's known-inert roster.
            ...
            if (wanSm_ > 1.0e-3f && T.mod != 3)
            {
                ...
                d += wanSm_ * 0.016f * fs_ * (wk_[v].st + bleedWalkRes (v));
            }
            ...
            if (vizTick_ == 0)
            {
                liveCents = (T.mod == 2 || ((C.flags & kStaticWalk) && v > 0))
                    ? statC_[v]
            ...
            // 🔴 fb425 — THIS GUARD WAS `T.mod == 2`, i.e. `Steady` ONLY, and that made
            //    ROSTER's claim for `Twofold`/`Static Pair` ("the walk is replaced by a
            //    fixed +-8-cent split") FALSE IN THE AUDIO. statC_ was computed for that
            //    Character and then read by nothing but viz_.voiceCents — the card drew a
            //    pitch offset the DSP never produced, which is fb373's geometry in
            //    miniature. Measured consequence: `Sway`, the Type's own hero knob, ran
            //    1.691 -> 1.691 cents across its entire travel on that Character (the
            //    detector's floor at both ends). The constant-ratio reader now runs
            //    wherever a static offset exists, so the shipped claim is true and the
            //    hero is alive.
            if (v > 0 && std::fabs (statC_[v]) > 0.02f && (T.mod == 2 || (C.flags & kStaticWalk)))
            {
    ...
    void procTwin (float lineL, float lineR, float limHi, float& wetL, float& wetR) noexcept
    {
        ...
        for (int p = 0; p < np; ++p)
        {
            ...
            if (wanSm_ > 1.0e-3f)
            {
                ...
                dLm += wanSm_ * 0.006f * fs_ * (wk_[p * 2].st     + bleedWalkRes (p * 2));
                dRm += wanSm_ * 0.006f * fs_ * (wk_[p * 2 + 1].st + bleedWalkRes (p * 2 + 1));
                tickWalk (wkp_[p], 0.22f + 0.75f * wanSm_);
                const float pj = wanSm_ * 0.45f * (wkp_[p].st + retR_ * resPw_[p]);
    ...
    void procBlur (float lineL, float lineR, float& wetL, float& wetR) noexcept
    {
       ...
       #else
        const float lsp = blurRot_.s + retR_ * resBs_, lcp = blurRot_.c + retR_ * resBc_;
       #endif
        ...
        wlk += 2.20f * retR_ * resBw_;                        // fb425 retrig residual
       #endif
        float a = m, b = m;
        for (int k = 0; k < nAP_; ++k)
    ...
    void procBands (float lineL, float lineR, float& wetL, float& wetR) noexcept
    {
        ...
        float sweep = gridRot_.s + retR_ * resGs_;            // fb425 retrig residual
        ...
        {   tickWalk (wkg_, 0.25f + 1.00f * wanSm_);
            sweep = clampf (sweep + 3.00f * wanSm_ * (wkg_.st + retR_ * resGw_), -2.0f, 2.0f); }
       #endif
    ...
    void applyField (float& wl, float& wr) noexcept
    {
        switch (field_)
        {
           ...
           #ifdef WIDEN_MUT_FLATFIELD
            // MUTATION (fb425): `Swap` and `Gather` collapse to `Straight`. Two of the six
            // options stop being physics; §V is the only thing in this cert that can see it.
            case 3: break;
    ...
    // 🔴 fb425 — AT fb424 THIS RESET ONLY ph_/res_/q_: the per-voice LFO phase, which is
    //    read by procVoices' mod-0 branch and by procTwin AND BY NOTHING ELSE. `Twofold`
    //    is driven by a band-limited random WALK (wk_), `Blur` by a rotator plus its own
    //    walk (blurRot_/wkb_), `Bands` by the crossover-grid rotator plus wkg_. None of
    //    them were touched, so firing the pill produced BIT-IDENTICAL OUTPUT on three of
    //    six Types — a front pill that did nothing on half the device, which is the
    //    dead-knob law. What "start over from a zeroed offset" means for a random walk is
    //    exactly that: a zeroed offset and a fresh target. For a rotator it is phase 0.
    //    Every one of those is a DISCONTINUITY IN A MODULATOR, so every one of them
    //    carries a residual — the state jumps, the offset does not.
    void fireRetrig() noexcept
    {
       #ifdef WIDEN_MUT_RETRIGLFO
        // MUTATION (fb424): the retrigger that only knew about the LFO. Twofold/Blur/Bands
        // come out BIT-IDENTICAL, which is what §P now measures.
        for (int v = 0; v < kMaxVoices; ++v)
        { res_[v] = std::sin (6.2831853f * ph_[v]); ph_[v] = 0.0f; q_[v] = 0.0f; }
        return;
       #endif
        for (int v = 0; v < kMaxVoices; ++v)
        {
            ...
            ph_[v]  = 0.0f;
            q_[v]   = 0.0f;
            // the WALK's residual is in walk units and multiplies the SAME delay depth the
            // LFO's does, so it is bled out under the SAME 0.5-sample-per-sample cap.
            resW_[v] += wk_[v].st;
            wk_[v].st = 0.0f; wk_[v].tg = 0.0f; wk_[v].ph = 0.0f;
            // the pair-placement walk is a GAIN, not a read position: linear 30 ms.
            resPw_[v] += wkp_[v].st;
            wkp_[v].st = 0.0f; wkp_[v].tg = 0.0f; wkp_[v].ph = 0.0f;
        }
        // `Blur` — the coefficient field's rotator and the walk that scatters it. Both are
        // ALLPASS COEFFICIENT modulators: 30 ms linear, the house smoothing time.
        resBs_ += blurRot_.s; resBc_ += blurRot_.c - 1.0f; resBw_ += wkb_.st;
        blurRot_ = Rot{}; wkb_.st = 0.0f; wkb_.tg = 0.0f; wkb_.ph = 0.0f;
        // `Bands` — the crossover GRID's rotator and its walk. A frequency, same law.
        resGs_ += gridRot_.s; resGw_ += wkg_.st;
        gridRot_ = Rot{}; wkg_.st = 0.0f; wkg_.tg = 0.0f; wkg_.ph = 0.0f;
        retR_ = 1.0f;
    }
    ...
    // fb425 — the WALK retrig residual, bled out under the read-position slew cap. The
    // cap is 0.5 samples of read movement per sample, and the residual reaches the read
    // head through BOTH walk paths at once (depG_ on Twofold, wanSm_*0.016*fs on the
    // wander), so the divisor is their sum: whichever path is live, the read position can
    // never move faster than half a sample per sample. Returns the residual still owed.
    inline float bleedWalkRes (int v) noexcept
    {
        if (resW_[v] == 0.0f) return 0.0f;
        const float step = 0.5f / std::max (1.0f, depG_[v] + wanSm_ * 0.016f * fs_);
        const float r = resW_[v];
        resW_[v] -= (r > 0.0f ? std::min (r, step) : std::max (r, -step));
        return r;
    }
    ...
    // 🔴 fb425 — THE RETRIGGER'S OTHER RESIDUALS. `res_` holds the LFO's; these hold the
    //    WALK's and the two rotators'. resW_ is in walk units and is bled out under the
    //    SAME read-position slew cap the LFO residual uses (it multiplies the same delay
    //    depth). retR_ is a LINEAR 30 ms ramp — never a one-pole, because a one-pole fade
    //    in float PARKS short of its target instead of arriving (the `Hear Mono` finding) —
    //    and it scales the residuals that live in the COEFFICIENT and GAIN domains, where
    //    30 ms is the house smoothing time and no read position is involved.
    float resW_[kMaxVoices]{}, resPw_[kMaxVoices]{};
    float retR_ = 0.0f, retK_ = 0.0f;
    float resBs_ = 0.0f, resBc_ = 0.0f, resBw_ = 0.0f, resGs_ = 0.0f, resGw_ = 0.0f;
