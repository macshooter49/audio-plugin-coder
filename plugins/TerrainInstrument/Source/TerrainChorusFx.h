...
private:
    ...
    static constexpr CharSpec CHAR[kNumTypes][kNumChars] =
    {
        ...
        { //  rate    dep    base   hf     comp   noise   grit   fast   drift  x1      x2     x3    flags
            ...
            { 0.75f, 1.80f, 1.25f, 0.96f, 1.00f, -60.f, 1.00f, 1.00f, 1.00f, 1.070f, 0.0f, 1.00f,  1.003f, 0 },            // Deep
        ...
        // ── June.  x1 = R clock ratio (1.000 = the true one-clock antiphase)
        {
            ...
            { 0.450f,1.80f, 2.00f, 0.96f, 1.00f, -60.f, 1.00f, 1.00f, 1.00f, 1.000f, 0.0f, 1.00f,  0.991f, 0 }             // Deep
        },
        // ── Pedal.  x1 = wet HP floor Hz (Thin)
        {
            ...
            { 0.40f, 1.00f, 1.00f, 0.96f, 1.20f, -58.f, 4.00f, 1.00f, 1.00f,   0.0f, 0.0f, 1.00f,  1.064f, 0 },            // Grit
            ...
            { 0.40f, 1.00f, 1.00f, 1.60f, 0.80f, -62.f, 0.96f, 1.00f, 1.00f, 300.0f, 0.0f, 1.00f,  1.014f, 0 }             // Thin
        },
        ...
        // ── Wow.  x1 = locked revolution warp Hz (0 = none), x2 = its depth (ms)
        {
            ...
            { 0.0f, 0.70f, 1.00f, 1.30f, 0.96f, -62.f, 0.96f, 2.00f, 0.30f, 0.00f, 0.0f, 1.0f,  1.016f, 0 },               // Pro Reel
        ...
        // ── Dark.  x1 = R stagger, x2 = clock jitter (ms), x3 = feedback floor
        {
            ...
            { 0.0f, 1.00f, 1.00f, 0.96f, 1.00f, -58.f, 1.10f, 1.00f, 1.00f, 1.31f, 0.4f, 0.00f,  1.053f, 0 },              // Cheap
    ...
    void recalc() noexcept
    {
        ...
        if (p.tempoSync)
        {
            // fb413 — SKIP INDEX 0. `Free` is what the Sync pill is for, so a synced knob must
            //   span divisions 1..19, never 0. This is the flanger's mapping, and it is now all
            //   three: the UI's own readout (grnDivIdx in index.html, shared with the granular
            //   Window and the tape Time) is 1 + round(t*(N-2)), so a 0..19 engine would have
            //   printed "1/8" while the DSP played 1/8D. That is fb373 in a new costume — the
            //   readout is not the path; both ends have to compute the SAME index.
            const int   idx   = clampi (1 + (int) std::lround (r01 * (kNumDivs - 2)), 1, kNumDivs - 1);
        ...
        // 🚨 fb413 — SYNC MUST WIN OVER THE CHARACTER LOCK. A locked-rate Character (`Solina`,
        //   `June I`, every one with rateHz > 0) pins the base rate and re-homes the knob to a
        //   +-1 octave trim, which is right for FREE running: it is how a locked Character avoids
        //   leaving a dead knob behind. But it was applied unconditionally, so it also swallowed
        //   the SYNCED rate — the pill lit, the card printed "1/8", and the chorus ran 0.513 Hz.
        //   Measured across the whole travel: 0.257 -> 1.026 Hz while the label swept 4 bar ->
        //   1/256. That is the sync law broken and fb373's lesson exactly: the readout was true
        //   about the knob and a lie about the DSP. A cross-device probe found it; no per-engine
        //   harness could, because each one only ever asked its own engine what it did.
        rateTg_ = clampf (p.tempoSync ? hz
                                      : ((C.rateHz > 0.0f) ? (C.rateHz * std::exp2 ((r01 - 0.5f) * 2.0f)) : hz),
                          0.01f, 140.0f);
