...
public:
    ...
    void setParams (const Params& pin) noexcept
    {
        ...
        if (p.tempoSync)
        {
            ...
            const int   idx   = clampi (1 + (int) std::lround (clamp01 (p.rate) * (kNumDivs - 2)), 1, kNumDivs - 1);
    ...
    float liveRateHz() const noexcept { return incTgt_ * fs_; }   // fb413 — the cross-device sync gate
