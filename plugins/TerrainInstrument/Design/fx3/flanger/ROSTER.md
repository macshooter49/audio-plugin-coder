...
| 4 | **Envelope** | A/DA Threshold · Bel BF-20 · BL-20 Env | The sweep's **control source** is the input follower, not a clock. Knee calibrated to the real bus: −38 dBFS floor, −26 dBFS knee, −14 dBFS full sweep. `Rate` scales the follower attack (60 → 1 ms); `Tail` owns its release. | Correlation of the comb position with the input envelope: **r = 0.99**. Every LFO Type: **r ≤ 0.10**. |
...
| 7 | **Tail** | 60 ms – 3 s, the feedback gate's release, applied **squared**. Envelope: also the follower release | 🔑 Flanger-specific because only this device runs ±0.995 regeneration. It kills the classic flanger sin — a patch that howls forever after note-off — while still letting you *play* a 3 s bloom. Measured **0.2 → 3.1 s** to −40 dB, monotonic, and every Type is silent 18 s after the input stops. |
