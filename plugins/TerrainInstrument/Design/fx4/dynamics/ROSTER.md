...
| P6 | **Cling** | 0 → 250 ms | Freezes the clamp before release runs — pump shaping, bass de-chatter. Bit-bypassed at 0. |
...
| P8 | **Burn** | 0 → 100 | How much the gain element distorts while it works. **Scaled by current GR**, so a patch with no compression is bit-clean at any Burn, and the dirt BREATHES with the compression. |
...
|---|---|---|---|
| (a) | Dynamic-range ratio of a **48 dB staircase** at Push 100 / Ratio 100 | ≤ 0.05 | **0.0188** (bypassed control: 1.0000) · mutant `compress-no-ceiling` (a polite 2:1 cap): **0.5069** |
...
| 4 | **Sheen** | Max's mandate — the "expensive top end" | X-High down to **1.55 kHz** and BOTH high-band thresholds moved up to the programme (T_dn −26, T_up −28 dBFS vs Over Top's −40/−46), so at programme level Sheen's high band is in its **UPWARD** lane while Over Top's is in its **DOWNWARD** lane; **0.35/8 ms** so the shimmer reads as texture not pumping | where the high band's net gain crosses into LIFT, read off the audio | **net lift begins at −3 dB of input for Sheen, −18 dB for Over Top — 15 dB apart.** ⚠️ this Type's discriminator CHANGED at fb423: it used to be "≥ 6 dB more air than Over Top", which is more of the same lift, not a different machine. The honest air figures are +18.70 vs +15.53 dB and they are printed as a `note`, not gated |
...
| **Sheen** | `Top Sheet` · `Higher Split` X-High ×2.6 — the sheen band shrinks to the top two octaves (at ×1.6 it measured 1.52× JND, a crossover nudge, not a mechanism) · `Lower Split` ×0.6 · `Glass Ceiling` T_up −10, knee 0, cap 10, clipper at +4 — a hard lid on the air · `Slow Shimmer` high-band times ×4 · `Fast Shimmer` ×0.25 · `Dark Source` high-band ears +6 dB, cap 30 · `Sheen Wall` upward ×1.25, T_up +8, cap 30 |
...
| **Surge** | `Tail Riser` · `Deep Riser` upward ×1.15, T_up +6, cap 36 · `Tied Rise` **one detector drives all three bands** — a loud low note stops the top from rising · `Fast Riser` ×0.25 · `Capped Riser` the lift is capped at **5 dB** · `Top Riser` high band ×0.3 in time, X-High ×0.7, cap 30 · `Mean Ears` a **60 ms RMS pre-average** — the riser stops hearing the waveform and rides the mean · `Riser Wall` T_up +10, cap 36 |
...
| pill | **Crest** | off / on | When a transient arrives, hold the UPWARD computer at unity for 10 ms so attacks keep their crest instead of being pre-inflated by the lift that was riding the gap in front of them. Default OFF. |
...
band) · `Free Pair` (two independent clamps) · `Mid-Side` (two clamps on a rotated basis, side
...
| Leg | Metric | Threshold | Measured |
|---|---|---|---|
| (a) | Residual envelope spread (p90−p10 of a 20 ms RMS envelope) at Amount 100, vs at Amount 50 | ≤ 65 % of it, and ≤ 7 dB absolute | probe 22.78 dB → **9.70 dB** at Amount 50 (= the preset) → **5.38 dB** at 100 (55 % of it) · mutant `ott-no-ceiling`: **9.70 dB, 100 % of it** |
| (b) | Dynamic-range ratio of a **46 dB staircase** at Amount 100 | ≤ 0.10 | **0.0742** (Amount 0 control: 0.9988) |
| (c) | A **−65 dBFS noise bed** lifted into a wall | ≥ +18 dB | **+33.1 dB** (Amount 0 control: +0.9 dB) |
| (d) | ... and a REAL dithered **−96 dBFS** bed stays inaudible | ≤ −70 dBFS | **−76.4 dBFS** · mutant `core-floor-gate`: **−44.4 dBFS** |
| (d) | ... while a **−72 dBFS** bed IS lifted (R11) | ≥ +30 dB | **+47.70 dB** |
...
"never shuts up" — and it is measured on a **real dithered bed**, never on digital zeros: the
device is feed-forward (`y = band·g`), so zero in gives zero out for any finite gain and the
−280 dBFS a zero probe returns is arithmetic, not evidence. It is printed as a `note`.
...
capitalised quoted string in `Source/` **and both sibling fx4 directories** — 3310 of them,
including `ui/public/index.html`, the `*_test.cpp` name tables and the leading-space labels
(`Motion`, `Route`) the first extractor skipped — and the harness checks **all 184** published
labels against it: every Type name, every Character name, every dropdown entry, every front and
back knob label, both pills and both device names, **read from the engine headers, not from this
file.** Plus every name against every other name across both devices.

🔴 **And downstream is gated EQUAL, not kept in sync by hand** (fb423 §Gate). Cert §1 reads
`ROSTER.md`, `compress-worklet.js` and `ott-worklet.js` off disk and requires the worklets' name
tables to EQUAL the header arrays string for string, requires every published label to appear
verbatim in this file, and requires ZERO occurrences of any RETIRED label in any of the three.
The harness no longer holds a label table of its own — it prints `backNames()[5]`, not `"Cling"`.
Two exemption lists exist and both are asserted to be exactly their stated size: the sibling
yields (`Gentle`, `Low Split` — 2 entries) and the rulings (`Auto` — 1 entry, RENAMES.md fb423
§SANCTIONED, the same law as the shipped Distortion `Auto` pill). A gate that can exempt itself
is not a gate. Six names moved
because of it:
...
| `Punch`, `Knee`, `Color`, `Hold`, `Link`, `Smooth`, `Slow`, `Dual`, `Bloom`, `Classic`, `Heavy`(ok), `Air` | shipped knob labels / Characters / the EQ agent's hero | **`Edge`, `Round`, `Burn`, `Cling`, `Tie`, `Average`, `Patient`, `Free Pair`, `Surge`, `Over Top`, —, `Top Lift`** |
...
- Both `Params` structs add **one bool** beyond contract §2 (`autoMakeup`, `crest`), the fx3
