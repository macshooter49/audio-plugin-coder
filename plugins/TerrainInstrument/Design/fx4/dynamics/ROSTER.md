...
|---|---|
| **Exact** | `Precise` the reference · `Soft Touch` the knee auto-widens by up to 26 dB below 12 dB of GR · `RMS Ears` forces a 10 ms mean-square detector · `Spike Ears` forces peak **+ 5 ms hold** (Exact's native ears are already peak, so a plain "Peak" character would have been a literal no-op — the harness measured the first draft's at **0.00× JND**) · `Deep Release` τR scales with GR · `Line Attack` a linear-in-dB attack RAMP, not an RC · `Silky` a critically-damped 2nd-order smoother (ζ = 1.0) · `Wobble` ζ = 0.42, a controlled release overshoot |
...
| **Two Band** | `Body Sparkle` · `Low Split` ~300 Hz · `High Split` ~1.4 kHz · `Hard Body` downward ×1.35, threshold −5, knee 0 · `Soft Body` downward ×0.6, knee 14 · `Sparkle Wall` upward ×1.2, T_up +6, cap 30, clipper at +4 · `Slow Twin` ×2.5 · `Fast Twin` ×0.2 |
...
| `Punch`, `Knee`, `Color`, `Hold`, `Link`, `Smooth`, `Slow`, `Dual`, `Bloom`, `Classic`, `Heavy`(ok), `Air` | shipped knob labels / Characters / the EQ agent's hero | **`Edge`, `Round`, `Heat`, `Latch`, `Tie`, `Average`, `Long`, `Twin`, `Surge`, `Over Top`, —, `Top Lift`** |
