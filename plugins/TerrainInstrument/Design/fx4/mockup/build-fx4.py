...
function makeSrc(){
  ...
  /* fb433 — SEEDED noise, reset per channel, so every source is MONO. A widener has
     nothing to show on already-decorrelated material; Math.random() per channel was
     handing it a wide source to widen. */
  ...
  for(var c=0;c<2;c++){ var d=b.getChannelData(c); sd=22222;
    for(var i=0;i<n;i++){ var t=i/sr, v=0;
      ...
      else if(kind===4){ /* Drums — the one with real GAPS, which is where OTT lives */
        var tb=t%%1.0, kt=tb%%0.5;
        var ke=Math.exp(-kt*20), kf=52*(1+3.2*Math.exp(-kt*42));
      ...
      else if(kind===5){ /* Pad — slow, dark, and quiet up top: the hardest test for air */
        [110,164.8,220,329.6].forEach(function(f,ix){
          var det=1+(c?1:-1)*0.0016*(ix+1);
      ...
      else if(kind===6){ /* Bell — long tail, so you hear what the release is doing */
        var bt=t%%2.0, be=Math.exp(-bt*2.6), mi=6.5*Math.exp(-bt*4.5);
        v=Math.sin(2*Math.PI*440*t+mi*Math.sin(2*Math.PI*622.3*t))*0.5*be; }
      else { /* Sweep — 100 Hz -> 8 k, exponential: shows a filter/EQ curve straight away */
        var r=80, T=4, ph2=100*T/Math.log(r)*(Math.pow(r,t/T)-1);
        v=Math.sin(2*Math.PI*ph2)*0.45; }
      d[i]=v; } }
