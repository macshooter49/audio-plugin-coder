...
class TerrainUtility extends AudioWorkletProcessor{
  constructor(){ super(); this.fs=sampleRate;
    ...
    this.gS=1; this.sideS=1; this.pL=1; this.pR=1; this.mixS=1; this.sumS=0; this.fl=1; this.fr=1; this.dimS=1; this.dcx=[0,0]; this.dcy=[0,0]; this.dcOn=0;
  ...
  process(inputs,outputs){ const inp=inputs[0], out=outputs[0]; if(!out||!out[0]) return true;
    ...
    for(let i=0;i<N;i++){ let xl=L?L[i]:0, xr=R?R[i]:xl; const dl=xl, dr=xr;
      ...
      if(this.dcOn){ const yl=xl-this.dcx[0]+0.9987*this.dcy[0]; this.dcx[0]=xl; this.dcy[0]=yl; xl=yl; const yr=xr-this.dcx[1]+0.9987*this.dcy[1]; this.dcx[1]=xr; this.dcy[1]=yr; xr=yr; }
