...
**fb423 — THE NAMES ROUND.** Six Types × eight Characters × six Fields, plus 3 front heroes + Mix
and 8 back knobs. The device is `Widen`. Engine: `TerrainWidenFx.h`. Certification:
`widen_cert.cpp` → `widen_cert_fb423.log` (**230 PASS / 5 FAIL** — the five are named in
`FINDINGS.md` §0, they are real, and they are reported rather than papered over).

🔴 **ALL 23 WIDEN ROWS of `Design/fx4/RENAMES.md` are applied verbatim** — 15 from the first table
and 8 from the fb423 second ruling. **Every knob label, Type name, Character name and Field name in
this document is published FROM `TerrainWidenFx.h`** — `typeNames()`, `charNames()`, `fieldNames()`,
`frontNames()`, `backNames()`, `pillNames()`, `dropdownNames()`, `voicesUnit()`. If this file and
the header ever disagree, **the header wins and this file is the bug** (FIXES §3) — and cert §S now
*proves* they agree instead of asking you to believe it: it reads this file and `widen-worklet.js`
off disk and asserts the worklet's seven name tables are entry-for-entry EQUAL to the header
arrays, that every card label appears here as a backticked literal, and that **no retired name
survives anywhere downstream**. There is no name authored in the worklet, in the roster or in the
harness; there are only copies that are gated equal.
...
**N copies of the input differing in pitch, in time, and in stereo placement.** The device name is
`Widen`, published by `deviceName()`. The two lineages
Max named are Types 0 and 1: `Throng` is the Roland JP-8000 Super Saw played as an *audio* effect
...
prose… Cert §O asserts it." **It did not.** §O measured `nV_` — a published integer — on `Throng`
and `Twofold` only, i.e. not on `Twin`, whose line count is a completely different expression, and
not on `Steady`. What is true now:

- `Voices` floors at **3 copies** on the three voice-crowd Types (`Throng`, `Steady`, `Twofold`) —
...
- On `Twin` the count is **lines**, and the Characters that name a count now deliver it exactly at
  ...
  sine-modulated control (`Tremble`, matched cents) at **12.5 %** and the arcsine law's closed-form
  prediction of 19 %.
...
|---|------|---------|---------------------------------------------|------------------------------|----------|
| 0 | **Throng** | Roland JP-8000 Super Saw, 1996 (Szabo's measured fan + mix law) | 3–8 micro-delay voices on one ring; each voice has its own **sine LFO at a scattered rate**; depth solved from the constant-cents law | **periodic** pitch motion, large modulation energy | modEng **1335 cents**, periodicity **0.869** |
...
- **`Ensemble`** (Solina/ARP triple chorus, 3-phase LFO pair) — **cut for two reasons, both hard.**
  ...
  violation across the rack. (2) Its mechanism is a *configuration* of Throng (locked-phase LFOs at
  two rates), not a different machine — exactly the blur CONTRACT §3.2 warns about. It survives
  where it belongs: **Throng / `Three Phase`**, which measures the largest Character distance in the
...
| `Tremble` | **triangle → sine.** The deliberate A/B that shows why the original is a triangle: the detune stops holding and starts breathing | 13.20 |
...
| `Quaver` | +0.55 of intrinsic Roam on the slaps | 4.18 |
...
| Character | What it re-wires | [dist] |
|---|---|---|
| `Lilt` | 17/29/41/53/23/35/47/61 ms, walk at 0.4–2.8 Hz | default |
...
|---|---|---|---|---|
| Hero 1 | **`Amount`** | Throng `Detune` · Twin `Depth` · Steady **`Cents`** · Twofold `Sway` · Blur `Wash` · Bands `Cleave` — **published by `frontNames(type)`** | per-Type, §4 | 🔴 **fb422 RE-VOICED (R11).** Measured on the OUTPUT SPECTRUM at Amount 100 %, Width 0.5 (unity): Throng **106.5 cents** · Twin **163.9** · Steady **144.3** · Twofold **55.8** · Blur corr **−0.87** · Bands corr **−0.55**. Control through the same engine at Amount 0: **1.7 cents / +1.000**. fb421 gave `Twin` **28 cents** — a mid-depth chorus on the Type Max named |
...
🔴 **fb423 — the MEMBER is `Params::axis`, the LABEL is `Field`.** CONTRACT §2 locks the
second-dropdown member name to `axis` ("R9: implement it exactly or integration breaks") and its
own comment shows the label varies per device (`Focus/Stereo/Route/...`). At fb422 this engine
declared `Params::field` and would not have compiled against the rack. The engine now publishes the
label too — `dropdownNames()` = { `Character`, `Field` } — so the card cannot print a word the DSP
disagrees with.
...
| # | Field | The matrix | Mono behaviour (measured, Throng, Mix 1.0) |
|---|---|---|---|
| 0 | `Straight` | identity | level −0.72 dB · worst notch −2.04 dB |
...
| 5 | `Gather` | side forced to 0: `L = R = m`. A pure thickener — the *most* mono-audible setting in the device | −0.72 dB · −2.04 dB |
...
| `Throng` | level −0.72 dB, notch **−2.04 dB** | **mono-safe**: the combs MOVE and average out |
...
```
SYN_WID_TYPE        choice(8)   6 live + 2 reserved-disabled   default 0 (Throng)
SYN_WID_CHARACTER   choice(8)   always 8; read back on /7 and clamp to the Type's list
SYN_WID_AXIS        choice(6)   Straight · Alternate · Orbit · Swap · Side Only · Gather
...
SYN_WID_RETRIG      bool        default OFF   -> Params::retrig
SYN_WID_MONO        bool        default OFF   -> Params::hearMono  (the `Hear Mono` pill)
