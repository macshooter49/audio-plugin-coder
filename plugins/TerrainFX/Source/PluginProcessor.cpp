...
//==============================================================================
TerrainFXAudioProcessor::TerrainFXAudioProcessor()
    : AudioProcessor (BusesProperties()
                       .withInput  ("Input",  juce::AudioChannelSet::stereo(), true)
                       .withOutput ("Output", juce::AudioChannelSet::stereo(), true)),
      // fb605 — "Parameters" is the APVTS ValueTree TYPE and it is written into every saved DAW
      // session; setStateInformation() below compares against it. Presentation renames never
      // touch it.
...
//==============================================================================
void TerrainFXAudioProcessor::initializePresets()
{
...
void TerrainFXAudioProcessor::loadPreset(int index)
{
...
int TerrainFXAudioProcessor::getPresetCount() const
{
    return static_cast<int>(presets.size());
}

juce::String TerrainFXAudioProcessor::getPresetName(int index) const
{
    if (index < 0 || index >= static_cast<int>(presets.size()))
        return {};
...
//==============================================================================
PresetData TerrainFXAudioProcessor::captureCurrentParams() const
{
    PresetData p;
...
int TerrainFXAudioProcessor::saveNewPreset(const juce::String& name, const juce::String& tag)
{
...
void TerrainFXAudioProcessor::renamePreset(int index, const juce::String& newName)
{
    if (index < 0 || index >= static_cast<int>(presets.size()))
        return;
...
void TerrainFXAudioProcessor::overwritePreset(int index)
{
    if (index < 0 || index >= static_cast<int>(presets.size()))
        return;
...
void TerrainFXAudioProcessor::deletePreset(int index)
{
...
juce::String TerrainFXAudioProcessor::getPresetTag(int index) const
{
    if (index < 0 || index >= static_cast<int>(presets.size()))
        return {};
...
void TerrainFXAudioProcessor::setPresetTag(int index, const juce::String& tag)
{
    if (index < numFactoryPresets || index >= static_cast<int>(presets.size()))
        return;
    presets[static_cast<size_t>(index)].tag = tag;
...
juce::String TerrainFXAudioProcessor::getCustomTags() const
{
    return customTags.joinIntoString(",");
}

void TerrainFXAudioProcessor::setCustomTags(const juce::String& commaSeparated)
{
    customTags.clear();
...
//==============================================================================
juce::AudioProcessorValueTreeState::ParameterLayout TerrainFXAudioProcessor::createParameterLayout()
{
...
//==============================================================================
void TerrainFXAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
...
void TerrainFXAudioProcessor::releaseResources()
{
    grainEngineL.reset();
    grainEngineR.reset();
...
bool TerrainFXAudioProcessor::isBusesLayoutSupported (const BusesLayout& layouts) const
{
...
void TerrainFXAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
...
//==============================================================================
juce::AudioProcessorEditor* TerrainFXAudioProcessor::createEditor()
{
    return new TerrainFXAudioProcessorEditor(*this);
}

bool TerrainFXAudioProcessor::hasEditor() const { return true; }

const juce::String TerrainFXAudioProcessor::getName() const { return JucePlugin_Name; }
bool TerrainFXAudioProcessor::acceptsMidi() const { return false; }
bool TerrainFXAudioProcessor::producesMidi() const { return false; }
bool TerrainFXAudioProcessor::isMidiEffect() const { return false; }
double TerrainFXAudioProcessor::getTailLengthSeconds() const { return 5.0; }

int TerrainFXAudioProcessor::getNumPrograms() { return static_cast<int>(presets.size()); }
int TerrainFXAudioProcessor::getCurrentProgram() { return currentPresetIndex.load(); }
void TerrainFXAudioProcessor::setCurrentProgram (int index) { loadPreset(index); }
const juce::String TerrainFXAudioProcessor::getProgramName (int index) { return getPresetName(index); }
void TerrainFXAudioProcessor::changeProgramName (int, const juce::String&) {}

//==============================================================================
void TerrainFXAudioProcessor::getStateInformation (juce::MemoryBlock& destData)
{
...
void TerrainFXAudioProcessor::setStateInformation (const void* data, int sizeInBytes)
{
...
//==============================================================================
juce::File TerrainFXAudioProcessor::getUserPresetsFile() const
{
    // ⚠️ fb605 — "Terrain" HERE IS NOT THE PRODUCT NAME, IT IS AN ADDRESS. The product became
    // "Terrain FX" this build (dir, targets, bundle, UI) — this folder did NOT move, because the
    // owner's saved user presets are already sitting in it (~/Library/Noizefield/Terrain/
    // UserPresets.xml). Repointing it at "Terrain FX" silently empties his preset list. Same law
    // as the plugin/manufacturer codes and the VST3 class UID: rename the presentation, never the
    // thing already written to disk. A move needs a MIGRATION, in the commit that owns it.
...
void TerrainFXAudioProcessor::saveUserPresetsToFile()
{
    ...
    // fb605 — PERSISTED TAG. loadUserPresetsFromFile() refuses any other tag name, so renaming
    // this to "TerrainFXUserPresets" would make every already-saved preset file unreadable.
    ...
    if (auto xml = root.createXml())
    {
        ...
        DBG("Terrain FX: saveUserPresetsToFile -> " + file.getFullPathName()
            + " (" + juce::String(presets.size() - numFactoryPresets) + " presets, "
            + (result ? "OK" : "FAILED") + ")");
    }
}

void TerrainFXAudioProcessor::loadUserPresetsFromFile()
{
    auto file = getUserPresetsFile();

    DBG("Terrain FX: loadUserPresetsFromFile -> " + file.getFullPathName()
    ...
    {
        DBG("Terrain FX: XML parse failed or wrong tag");
        return;
    }
    ...
    DBG("Terrain FX: loaded " + juce::String(loaded) + " user presets from disk, total=" + juce::String(presets.size()));
}

//==============================================================================
// Rolling Capture Buffer
//==============================================================================
float TerrainFXAudioProcessor::getCaptureAvailableSeconds() const
{
    return captureBuffer.getAvailableSeconds();
}

juce::String TerrainFXAudioProcessor::getLastCaptureFilePath() const
{
    return lastCaptureFilePath;
}

void TerrainFXAudioProcessor::exportCapture(int durationSeconds)
{
    ...
    // fb605 — kept as "Terrain" on purpose: the owner already has 70+ captures in this folder and
    // DAW projects referencing them by path. The product name moved; his files did not.
...
{
    return new TerrainFXAudioProcessor();
}
