...
**Framework Routing:**
- If `ui_framework == webview`: use templates from `..agent/templates/webview/`
- If `ui_framework == visage`: use templates from `..agent/templates/visage/` and **do not** generate HTML
...
Use templates from `..agent/templates/visage/` and the shared host in `common/VisageJuceHost.h`.
...
**CRITICAL:** When implementing WebView plugins, you MUST verify ALL 8 points below. Use templates from `..agent/templates/webview/` and run validation script.
...
**Copy templates from:** `..agent/templates/webview/`
...
2. Check templates in `..agent/templates/webview/`
